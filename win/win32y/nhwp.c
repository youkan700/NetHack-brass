...
struct {
	...
	{ "Arial",	18, &iflags.wc_font_status,	      &iflags.wc_fontsiz_status,
			     17, &iflags.wc_foregrnd_status,  &iflags.wc_backgrnd_status  },	/* HPBAR   */
	{ "Arial",	18, &iflags.wc_font_menu,	      &iflags.wc_fontsiz_menu,
			      0, 0,			      0,			  }	/* BASE    */
#else
	{ "Terminal",	     18, &iflags.wc_font_map,	      &iflags.wc_fontsiz_map,
...
void nhw_printglyph(int x, int y, int chr, int fc, int bc) {
	...
	nhwid = (x == -1) ? y : NHW_WINID_MAP;

	if (tiledMapFont != NULL) {
	    BYTE *psrc, *pdst;
	    BYTE fr, fg, fb, br, bg, bb;
	    int i, j, w, h;
	    w = nhwi[NHW_WINID_MAP].fw;
	    h = nhwi[NHW_WINID_MAP].fh;
	    psrc = &tiledMapFont[chr*w*h];
	    fr = (colortable[fc] & 0x0000FF);
	    fg = (colortable[fc] & 0x00FF00) >>  8;
	    fb = (colortable[fc] & 0xFF0000) >> 16;
	    br = (colortable[bc] & 0x0000FF);
	    bg = (colortable[bc] & 0x00FF00) >>  8;
	    bb = (colortable[bc] & 0xFF0000) >> 16;
	    if (nhwid == NHW_WINID_MAP) {
		yy = nhwi[NHW_WINID_MAP].winy + y * h;
	    } else {
		yy = nhwi[nhwid].winy + nhwi[nhwid].cy;
	    }
	    for (i = yy; i < yy+h; i++) {
		if (nhwid == NHW_WINID_MAP) {
		    xx = nhwi[NHW_WINID_MAP].winx + x * w;
		} else {
		    xx = nhwi[nhwid].winx + nhwi[nhwid].cx;
		}
		pdst = &bkbuf.lpBMP[(xx + ((bkbuf.h-i-1)*bkbuf.w))*4];
		for (j = xx; j < xx+w; j++) {
		    if (*psrc++) {
			*pdst++ = fb;
			*pdst++ = fg;
		...
		xx = nhwi[nhwid].winx + nhwi[nhwid].cx;
		yy = nhwi[nhwid].winy + nhwi[nhwid].cy;
		nhwi[nhwid].cx += nhwi[NHW_WINID_MAP].fw;
	    }
...
/*------------------------------------------------
	HP bar
  ------------------------------------------------*/
void nhw_drawhpbar(int x, int y, int hp, int mhp) {
	HDC hdc;
	int i;
	int xx, yy;
	BYTE *pdst1, *pdst2;
	...
	pdst1 = &bkbuf.lpBMP[(xx + ((bkbuf.h-yy-1)*bkbuf.w))*4];
	pdst2 = &bkbuf.lpBMP[(xx + ((bkbuf.h-yy-2)*bkbuf.w))*4];
	for (i=0; i<(nhwi[NHW_WINID_MAP].fw*hp/mhp); i++) {
	    *pdst1++ = 0x00; // blue
	    *pdst1++ = 0xFF; // green
	    *pdst1++ = 0x00; // red
	    *pdst1++ = 0x00;
	...
	hdc = GetDC(bkbuf.hWnd);
	BitBlt(hdc, xx, yy, nhwi[NHW_WINID_MAP].fw, 2,
	       bkbuf.hdcMem, xx, yy, SRCCOPY);
	ReleaseDC(bkbuf.hWnd, hdc);
}
#if 0
void nhw_drawhpbar(int index, int chr, int fc, int bc, int hpnow, int hpprev, int hpmax) {
	int x0, x1, x2, x3;
	HBRUSH tmpb;
	RECT r;
	nhwi[NHW_WINID_HPBAR].cx = index * 96;
	/* print glyph character */
	nhw_printglyph(-1, NHW_WINID_HPBAR, chr, fc, bc);
	x0 = nhwi[NHW_WINID_HPBAR].winx + nhwi[NHW_WINID_HPBAR].cx;
	x1 = x0 + (hpnow  * 60 / hpmax);
	x2 = x0 + (hpprev * 60 / hpmax);
	x3 = x0 + 60;
	SetRect(&r, x0  , nhwi[NHW_WINID_HPBAR].winy,
		    x1+1, nhwi[NHW_WINID_HPBAR].winy + nhwi[NHW_WINID_HPBAR].fh);
	/* Remaining HP */
	tmpb = CreateSolidBrush(RGB(0x00, 0xff, 0x00)); /* bright green */
	FillRect(getHDCNHW(NHW_WINID_HPBAR), &r, tmpb);
	DeleteObject(tmpb);
	/* Newly damaged */
	if (x2 > x1) {
	    r.left  = x1;
	    r.right = x2+1;
	    tmpb = CreateSolidBrush(RGB(0xff, 0x00, 0x00)); /* bright red */
	    FillRect(getHDCNHW(NHW_WINID_HPBAR), &r, tmpb);
	    DeleteObject(tmpb);
	} else
	    x2 = x1;
	...
	if (x3 > x2) {
	    r.left  = x2;
	    r.right = x3+1;
	    tmpb = CreateSolidBrush(colortable[CLR_BLACK]);
	    FillRect(getHDCNHW(NHW_WINID_HPBAR), &r, tmpb);
	    DeleteObject(tmpb);
	}
	nhwi[NHW_WINID_HPBAR].cx += 60;
	updateNHW(NHW_WINID_HPBAR);
}
#endif
...
void init_nhw(HWND hwnd) {
	...
	for (i=0; i<NHW_WINID_MAX; i++) {
		...
		l = strlen(fnam);
		if (l > 4 &&
		    (!strcmpi(fnam + l - 4, ".gif") ||
		     !strcmpi(fnam + l - 4, ".bmp"))) {
		     tiledMapFont = nhwLoadTiledFont(fnam, &(nhwi[i].fw), &(nhwi[i].fh));
		     nhwi[i].font = NULL;
		     if (tiledMapFont != NULL) goto skipinitfont;
		}
	    }
...
skipinitfont:
	...
	h = nhwi[NHW_WINID_MAP].fh * 21/*ROWS*/ + WSPACING +
	    ...
	    nhwi[NHW_WINID_STATUS].fh*2 /* status line */ + WSPACING +
	    max(nhwi[NHW_WINID_HPBAR].fh, nhwi[NHW_WINID_MAP].fh) /* hpbar line */;
	...
	nhwi[NHW_WINID_HPBAR].winw = w;
	nhwi[NHW_WINID_HPBAR].winh = max(nhwi[NHW_WINID_HPBAR].fh, nhwi[NHW_WINID_MAP].fh);
	nhwi[NHW_WINID_HPBAR].winx = WSPACING;
	nhwi[NHW_WINID_HPBAR].winy = nhwi[NHW_WINID_STATUS].winy + nhwi[NHW_WINID_STATUS].winh + WSPACING;
...
void tini_nhw(void) {
	...
	for (i=0; i<NHW_WINID_MAX; i++) {
	    if (nhwi[i].font    != NULL) DeleteObject(nhwi[i].font);
	    if (nhwi[i].bgbrush != NULL) DeleteObject(nhwi[i].bgbrush);
	}
	if (tiledMapFont != NULL) GlobalFree(tiledMapFont);
}
