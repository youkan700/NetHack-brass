...
#define	NHW_WINID_HPBAR		5
#define	NHW_WINID_BASE		6
#define	NHW_WINID_USER		7
#define	NHW_WINID_MAX		7
...
typedef struct nhwWinInfoTag {
	...
	int	cx;	/* cursor x */
	int	cy;	/* cursor y */
	int	cx0;	/* cursor x leftmost position (for drawing margin) */
	int	fc;	/* default forecolor */
	int	bc;	/* default backcolor */
} nhwWinInfo;
...
void  win32y_print_glyph(winid, xchar, xchar, int);
...
void  win32y_draw_hpbar(winid, int, int, int, int);
...
char  getyn_core(int, const char *, char, int *, char);
int   getext_core(int, char *, int, char * (*)(char *));

LPBYTE nhwLoadTiledFont(char *, int *, int *);
