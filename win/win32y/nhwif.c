...
void win32y_draw_hpbar(winid win, int x, int y, int hp, int mhp) {
	if (mhp <= 0) return;
	if (hp < 0 || hp > mhp) return;
	if (win == NHW_WINID_MAP) {
	    nhw_drawhpbar(x, y, hp, mhp);
	}
}
/* UNDER CONSTRUCTION */

/* print_glyph */
void win32y_print_glyph(winid win, xchar x, xchar y, int glyph) {
	int chr, fc, bc, sp;
	bc = 16;
	if (win == NHW_WINID_MAP) {
	    ...
	    if ((glyph & GLYPH_TYPE_MASK) == GLYPH_TYPE_MON) {
		int hp, mhp;
		if (x == u.ux && y == u.uy) {
		    if (Invis) {
			return;
