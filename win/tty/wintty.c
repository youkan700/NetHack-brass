...
void win32y_draw_hpbar(winid win, int index, int glyph, int hpnew, int hpold) {
	...
	int i;
	if (win == WIN_HPBAR) {
	    mapglyph(glyph, &chr, &fc, &sp, 0, 0);
	    tty_curs(WIN_HPBAR, index * 10 + 1, 0);
#ifdef TEXTCOLOR
	    if (fc != ttyDisplay->color) {
		if(ttyDisplay->color != NO_COLOR)
		    term_end_color();
		ttyDisplay->color = fc;
		if(fc != NO_COLOR)
	    ...
	    (void) putchar(chr);
#ifdef TEXTCOLOR
	    if (CLR_BRIGHT_GREEN != ttyDisplay->color) {
		if(ttyDisplay->color != NO_COLOR)
		    term_end_color();
		ttyDisplay->color = CLR_BRIGHT_GREEN;
		term_start_color(CLR_BRIGHT_GREEN);
	    }
#endif /* TEXTCOLOR */
	    for (i=0; i<((hpnew + 0x7F)>>7); i++) (void) putchar('*');
#ifdef TEXTCOLOR
	    term_end_color();
	    ttyDisplay->color = CLR_ORANGE;
	    term_start_color(CLR_ORANGE);
#endif /* TEXTCOLOR */
	    for (; i<((hpold + 0x7F)>>7); i++) (void) putchar('*');
#ifdef TEXTCOLOR
	    term_end_color();
	    ttyDisplay->color = CLR_BLACK;
	    term_start_color(CLR_BLACK);
#endif /* TEXTCOLOR */
	    for (; i<8; i++) (void) putchar('*');
#ifdef TEXTCOLOR
	    /* turn off color as well, ATR_INVERSE may have done this already */
	    if(ttyDisplay->color != NO_COLOR) {
		term_end_color();
