...
do_objs()
{
	...
	for(i = 0; !i || objects[i].oc_class != ILLOBJ_CLASS; i++) {
		...
		/* if a description name has some functionality, list it up */
		if (objnam = tmpdup(OBJ_DESCR(objects[i]))) {
		    if (class == ARMOR_CLASS &&
			(!strcmp(objnam, "riding boots") ||
			 !strcmp(objnam, "mud boots") ||
			 !strcmp(objnam, "snow boots") ||
			 !strcmp(objnam, "riding gloves") ||
			 !strcmp(objnam, "visored helmet") ||
			 !strcmp(objnam, "tattered cape"))) {
		    } else if (class == POTION_CLASS &&
			(!strcmp(objnam, "milky") ||
			 !strcmp(objnam, "smoky"))) {
			Strcat(objnam, "_potion");
		    } else objnam = 0;
		    if (objnam) {
			for (c = objnam; *c; c++)
			    if (*c >= 'a' && *c <= 'z') *c -= (char)('a' - 'A');
...
do_japanese(void)
{
	...
	ntechs = MAXTECH;
	strbuf = (char *)malloc(128 * (nobjs + narts + ntechs)); /* 64 for name, 64 for descr */
	if (strbuf == NULL) {
	...
	while (fgets(in_line, sizeof in_line, ifp)) {
		...
		case 'T':
		    for (i = 1; i< ntechs; i++) {
			if (!strcmp(&enam[1], tech_names[i])) {
			    Strcpy(&strbuf[128*(nobjs+narts+i)], jnam);
			}
		    }
		    continue;
		default:
	...
	Fprintf(ofp,"};\n\n");
	Fprintf(ofp,"NEARDATA char *jtech_names[] = {\n");
	for (i = 0, s = &strbuf[128*(nobjs+narts)]; i < ntechs; i++, s += 128) {
	    if (*s)
		Fprintf(ofp,"    \"%s\",\n", s);
	    else {
		Fprintf(ofp,"    (char *)0,\n");
		if (i)
		    Fprintf(stdout, "Warning: No Japanese technique name for '%s'\n", tech_names[i]);
	    }
	}
	Fprintf(ofp,"};\n\n");
	free(strbuf);
...
do_objdesc(void)
{
	...
	obuf = (long *)malloc(sizeof(long) * (nobjs*2 + narts + ntechs));
	if (obuf == NULL) {
		perror("Out of memory");
		Fclose(ifp);
		exit(EXIT_FAILURE);
	}
	for (i = 0; i < nobjs*2+narts+ntechs; i++) obuf[i] = 0;
	...
	(void) fwrite((genericptr_t)obuf, sizeof(long), nobjs*2+narts+ntechs, ofp);
	woffset = ftell(ofp);
	...
	while (fgets(in_line, sizeof in_line, ifp)) {
		...
		case 'T':
		    for (i = 1; i< ntechs; i++) {
			if (!strcmp(&in_line[1], tech_names[i])) {
			    do_objdesc_sub((long)(nobjs*2 + narts + i));
			    stt = 1;
			    break;
			}
		    }
		    if (i < ntechs) continue;
		    goto notfnd;
		    break;
		default:
		    continue;
	    }
	    for (fnd = 0, i = 1; i < nobjs; i++) {
		if (objects[i].oc_class != class) continue;
		if (obj_descr[i].oc_name &&
		...
		if (obj_descr[i].oc_descr &&
		    ...
		    stt = 1;
		    fnd = 1;
		    continue; /* description name is not unique... */
		}
	    }
	    if (fnd) continue;
notfnd:	    Fprintf(stderr, "Error in line %4d: Cannot find \'%s\'\n", line_cnt, &in_line[1]);
	    Fclose(ifp);
	    Fclose(ofp);
	    free(obuf);
	    Unlink(filename);
	...
	for (i = 0; i < nobjs*2+narts+ntechs; i++ ) {
		offset = obuf[i];
