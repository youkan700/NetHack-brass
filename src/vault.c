...
register boolean forceshow;
{
	...
	struct egd *ep = EGD(grd);

	if (!on_level(&(ep->gdlevel), &u.uz)) return TRUE;

	while((fcbeg = ep->fcbeg) < ep->fcend) {
		fcx = ep->fakecorr[fcbeg].fx;
		fcy = ep->fakecorr[fcbeg].fy;
		...
		levl[fcx][fcy].typ = ep->fakecorr[fcbeg].ftyp;
...
int x, y;
{
	...
	for(fci = ep->fcbeg; fci < ep->fcend; fci++)
		if(x == ep->fakecorr[fci].fx &&
				y == ep->fakecorr[fci].fy)
			return(TRUE);
	return(FALSE);
}
...
fnd:
	...
	if(IS_WALL(levl[x][y].typ))
	    ep->fakecorr[0].ftyp = levl[x][y].typ;
		...
		ep->fakecorr[0].ftyp = TLCORNER;
	    else if(x == hix+1 && y == lowy-1)
		ep->fakecorr[0].ftyp = TRCORNER;
	    else if(x == lowx-1 && y == hiy+1)
		ep->fakecorr[0].ftyp = BLCORNER;
	    else if(x == hix+1 && y == hiy+1)
		ep->fakecorr[0].ftyp = BRCORNER;
	    else if(y == lowy-1 || y == hiy+1)
		ep->fakecorr[0].ftyp = HWALL;
	    else if(x == lowx-1 || x == hix+1)
		ep->fakecorr[0].ftyp = VWALL;
	}
	levl[x][y].typ = DOOR;
...
paygd()
{
	...
	if (u.uinvault) {
	    ...
	    gx = rooms[ep->vroom].lx + rn2(2);
	    gy = rooms[ep->vroom].ly + rn2(2);
#ifndef JP
	    Sprintf(buf,
