...
{
    boolean mon_mimic = (mon->data->mlet == S_MIMIC && mon->m_ap_type != M_AP_NOTHING);
    int sensed = mon_mimic &&
	(Protection_from_shape_changers || sensemon(mon));
    /*
     * Animated objects are special: Their appearance is an object in mon->mw.
     */
    if (is_animobj(mon->data)) {
	int num;
	if (!mon->minvent || !mon->mw || mon->minvent != mon->mw) {
	    /* Called from makemon()... just ignore it */
	    return;
	}
	if (sightflags == DETECTED) {
	    num = detected_obj_to_glyph(mon->mw);
	} else if (mon->mtame && !Hallucination) {
	    num = pet_obj_to_glyph(mon->mw);
	} else {
	    num = obj_to_glyph(mon->mw);
	}
	show_glyph(x,y,num);
	return;
    }
    ...
    int x, y, glyph;
{
	...
	impossible("show_glyph:  bad pos %d %d with glyph %08X.",
						x, y, glyph);
	return;
    }
    ...
    if (levl[x][y].mark) glyph |= GLYPH_ATTR_GOODPOS;
    ...
    int loc;
{
    ...
    return ((int) what_mon(mnum) | ((loc - S_sw_tl) << GLYPH_SUB_SHIFT)) | GLYPH_TYPE_SWALLOW;
}
...
//    dx = (dx == dy) ? 2 : (dx && dy) ? 3 : dx ? 1 : 0;
    ...
    return ((int) ((beam_type << 4) | zsym[dxdy2dir(dx, dy) & 3])) +
