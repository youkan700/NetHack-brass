...
domonability()
{
	...
	else You(E_J("don't have a special ability in your normal form!",
		     "特殊な能力を持っていない！"));
	return 0;
}
...
{
#ifdef WIZARD
	if (wizard && (yn("迷宮全体のステータスを表示しますか？") == 'y'))
	    return wiz_where();
	else
#endif
	dooverview();
...
int final;	/* 0 => still in progress; 1 => over, survived; 2 => dead */
{
	...
	if (Lev_at_will) {
	    if (Levitation) E_J(you_are("levitating, at will"),you_have("自分の意思で浮遊して"));
	    else if (ELevAtWill) you_can(E_J("levitate, at will","自分の意思で浮遊することが"));
...
//	    if(uright && uright->otyp == RIN_PROTECTION) prot += uright->spe;
	...
	if (Upolyd) {
	    if (u.umonnum == u.ulycn) Strcpy(buf, E_J("in beast form","獣の姿をとって"));
		     ...
		     Sprintf(buf, "%sに変化して", JMONNAM(u.umonnum)));
#ifdef WIZARD
...
struct ext_func_tab extcmdlist[] = {
	...
	{(char *)0, (char *)0, donull, TRUE},	/* xdat */
#endif
...
static const struct ext_func_tab debug_extcmdlist[] = {
	...
	{"uuddlrlrba", "gain full stats", wiz_gain_fullstat, TRUE},
	{"roomlist", "show list of rooms", wiz_roomlist, TRUE},
	{"cloudlist", "show list of clouds", wiz_cloud_list, TRUE},
	{"animobj", "animobj test", wiz_objdesc, TRUE},
	{"xdat", "show extra data", wiz_xdat, TRUE},
...
playersteal()
{
	...
	if(MON_AT(x, y)) {
		...
		if (!mtmp->minvent || is_animobj(mtmp->data)) {
...
cleanup:
		...
		for(mwatch = fmon; mwatch; mwatch = mwatch->nmon) {
			...
			mwatch->mcanmove &&
			(mwatch->mnum == PM_WATCHMAN ||
			 mwatch->mnum == PM_WATCH_CAPTAIN) &&
...
return 0;
	...
	if (uwep) {
	    otmp = uwep;
	    obj_extract_self(uwep);
	} else {
	    otmp = mksobj(rn2(2) ? LONG_SWORD : SCIMITAR, FALSE, FALSE);
	}
	mpickobj(mtmp, otmp);
	...
	tmpdata = mons[PM_ANIMATED_OBJECT]; /* template */
	w = weight(otmp);
	tmpdata.mmove = (w >= 100) ? 12 - (w-100)/20 : 18 - w/15;
	add_xdat_mon(mtmp, XDAT_PERMONST, &tmpdata);
	mtmp->data = get_xdat_mon(mtmp, XDAT_PERMONST);
	pline("Speed: %d", mtmp->data->mmove);
