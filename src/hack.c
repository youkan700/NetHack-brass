    ...
    xchar x, y;
{
	...
	You("%s%sを齧りはじめた。",
	    ...
	    (boulder || IS_TREES(lev->typ) || lev->typ == IRONBARS) ? "" : "の穴");
#endif /*JP*/
...
collision:
	...
	if (closed_door(x,y)) {
		...
		if (mode == DO_MOVE) {
			...
			} else if (flags.autoopen && !nohands(youmonst.data)) {
			    /* try to open the door */
			    coord cc;
			    cc.x = x;
			    cc.y = y;
			    flags.door_opened = open_door(&cc);
			} else {
			    pline(E_J("That door is closed.","扉は閉じている。"));
			}
		    }
    ...
    } else if (tmpr->typ == WATER && Wwalking && Is_waterlevel(&u.uz)) {
	if (mode == DO_MOVE)
	    Your(E_J("%s doesn't sink into water!","%sが水の中に入らない！"), body_part(FOOT));
	return FALSE;
    }
    if (dx && dy
    ...
    /* Avoid crossing a long worm tail diagonally */
    if (dx && dy) {
	struct monst *mtmp;
	mtmp = m_at(ux,y);
	if (mtmp && m_at(x,uy) == mtmp) {
	    if (mode == DO_MOVE)
		You_cant(E_J("cross over %s.","%sの巨体を乗り越えられない。"), mon_nam(mtmp));
	    return FALSE;
	}
    }
...
domove()
{
	...
	if(u.uswallow) {
		...
		if (on_ice) {
		    if ((uarmf && objects[uarmf->otyp].oc_descr_idx == SNOW_BOOTS)
...
stillinswamp:
	...
	if (levl[u.ux][u.uy].typ == IRONBARS &&
	    ...
	    You(E_J("dissolves the iron bars!","鉄格子を溶かした！"));
...
inv_weight()
{
	...
	while (otmp) {
		/* worn armor is slightly lighter */
		if (is_worn(otmp) && is_heavy_armor(otmp)) {
		    wt += (otmp->owt) * (200 - P_SKILL(P_ARMORED_COMBAT)) / 200;
		} else
