...
struct cloud *ctmp;
{
	...
	case STINKING_CLOUD:
	    if (ctmp->density > 5) {
		ctmp->glyph = cloud_to_glyph(S_densepcloud);
		ctmp->doesblock = 0;
	    } else {
		ctmp->glyph = cloud_to_glyph(S_thinpcloud);
		ctmp->doesblock = 0;
	    }
	    break;
	default:
...
run_cloud()
{
	...
	switch (ctmp->ctyp) {
		...
		if (u.ux == ctmp->cx && u.uy == ctmp->cy &&
		    ...
		    if (nonliving(youmonst.data) || Breathless)
			break;
		    if (!Blind && !is_full_resist(POISON_RES))
			make_blinded(1L, FALSE);
		    if (!Poison_resistance) {
#ifndef JP
			pline("%s is burning your %s!", Something, makeplural(body_part(LUNG)));
...
run_cloud_mon:
		  ...
		  do {
			...
			if (mtmp->mnum == PM_GREEN_DRAGON ||
			    mtmp->mnum == PM_BABY_GREEN_DRAGON)
				continue;
			if (cansee(mtmp->mx, mtmp->my))
...
#ifdef SHOWDMG
				if (flags.showdmg && canseemon(mtmp))
				    printdmg(tmp);
#endif
			    }
			}
			if (!DEADMONSTER(mtmp) && ctmp->madebyu) {
