...
register struct obj *otmp;
{
	...
	if (carried(otmp)) {
		...
		otmp->bypass = 1;		/* hack to prevent merge */
		otmp = addinv(otmp);
		otmp->bypass = 0;
	    }
	}
...
register struct permonst *ptr;
{
	...
	switch (type) {
		case POISON_RES:
			if ((ptr->mnum == PM_KILLER_BEE ||
			     ptr->mnum == PM_SCORPION) && !rn2(4))
				chance = 1;
			else
				chance = 25;
			break;
...
/* STEHPEN WHITE'S NEW CODE */
		...
		if (youmonst.data->mlet != S_MIMIC && !Unchanging) {
			...
			Upolyd ? JMONNAM(u.umonnum) : urace.noun);
#endif /*JP*/
	...
	register struct obj *otmp;
{
	...
	if (victual.tainted) {
		...
		if (Sick_resistance) {
			...
			if (!uniq)
			    Sprintf(buf, E_J("rotted %s","腐った%sを食べて"), corpse_xname(otmp,TRUE));
			else
#ifndef JP
			    Sprintf(buf, "%s%s rotted corpse",
				    ...
				    s_suffix(mons[mnum].mname));
#else
			    Sprintf(buf, "%sの腐った死体を食べて", JMONNAM(mnum));
#endif /*JP*/
...
#else
		if (!retcode) pline("死体は完全に腐り果ててしまった。");
#endif /*JP*/
		if (carried(otmp)) useup(otmp);
		else useupf(otmp, 1L);
		retcode = 2;
...
//			delayed_killer = killer_buf;
			set_delayed_killer(STONED, KILLED_BY_AN, killer_buf);
		    }
		}
		break;
...
int mnum;
{
	return (((touch_petrifies(&mons[mnum]) || mnum == PM_MEDUSA) &&
...
//	    if (otmp->oclass == WEAPON_CLASS && otmp->opoisoned) {
		...
		if (otmp->otyp == IRON_CHAIN && otmp->spe == 1) {
		    pline(E_J("This iron bars are delicious!","この鉄格子はうまい！"));
		} else {
