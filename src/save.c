...
register int fd, mode;
{
	...
	bwrite(fd, (genericptr_t) delayed_killers, sizeof delayed_killers);
	bflush(fd);
}
...
register struct obj *otmp;
{
	...
	while (xl--) {
	    ...
	    if (otmp->omon)
		savemonchn(fd,otmp->omon,mode);
	    if (otmp->oextra)
		savexdatchn(fd,otmp->oextra,mode);
	    if (release_data(mode)) {
...
register struct monst *mtmp;
{
	...
	while (xl--) {
	    ...
	    if (mtmp->mextra)
		savexdatchn(fd,mtmp->mextra,mode);
	    mtmp = mtmp2;
	}
	if (release_data(mode))
	    while (mtmp) {
