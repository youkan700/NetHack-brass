...
static int kbprefix;
void
bullet_killername(mon, obj, name)
struct monst *mon;
struct obj *obj;
const char *name;
{
	...
	if (!obj && !name) panic("bullet_killername: both obj and name null?");

	kbprefix = KILLED_BY; /* default */

	if (!mon && !obj && name) {
	    /* maybe special killer */
	    strcpy(killer_buf, name);
	    kbprefix = NO_KILLER_PREFIX;
	    return;
	}

	if (!mon) {
	    /* assume it is thrown from a trap */
	    int tt;
	    switch (obj->otyp) {
		case ARROW:
		    tt = ARROW_TRAP;
		...
		case DART:
		    ...
		    break;
		case BOULDER:
		    tt = ROLLING_BOULDER_TRAP;
		    break;
		default:
		    tt = NO_TRAP;
		    impossible("bullet_killername: Unknown shooter trap?");
		    break;
	    }
	    Sprintf(killer_buf, "%sに\tかかって", defsyms[trap_to_defsym(tt)].explanation);
	    return;
	}
	...
	if (mon == &youmonst) {
	    /* thrown by yourself */
	    Sprintf(killer_buf, "自分で%s%sに当たって死んだ", verb, name ? name : killer_xname(obj));
	} else {
	    /* thrown by a monster */
	    setup_killername(mon, killer_buf);
	    Sprintf(eos(killer_buf), "が%s%sで", verb, name ? name : killer_xname(obj));
	    kbprefix = KILLED_SUFFIX;
	}
}
#endif /*JP*/
...
//	    singleobj->oeaten = mon->m_id; /* mark the shooter */
...
#ifdef JP
		    bullet_killername(mon, singleobj, 0);
#endif /*JP*/
