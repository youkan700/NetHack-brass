	...
	for (x = 0; x < sizeof(mptr->rooms); x++) {
		if (mptr->rooms[x] & MSR_SEEN) {
			if (rooms[x].rtype >= SHOPBASE) {
				if (!mptr->feat.nshop)
					mptr->feat.shoptype = rooms[x].rtype - SHOPBASE;
...
dooverview()
{
	...
	boolean printdun;
	int lastdun = 0;
	int ct = 0;	/* fix display bug when undiscoverd [Sakusha]*/
	...
	for (mptr = mapseenchn; mptr; mptr = mptr->next) {
		...
		if (interest_mapseen(mptr)) {
			ct ++;	/* fix display bug when undiscoverd [Sakusha]*/
	...
	/* fix display bug when undiscoverd [Sakusha]*/
	if(!ct) {
/*JP
...
#else
	case BR_PORTAL:	 return closed_portal ? "•Â‚´‚³‚ê‚½–‚–@‚Ì–å" : "–‚–@‚Ì–å";
	case BR_NO_END1: return "“ü‚èŒû";
	case BR_NO_END2: return (br->end1_up) ? "ˆê•û’Ês‚Ìã‚èŠK’i" : "ˆê•û’Ês‚Ì‰º‚èŠK’i";
	case BR_STAIR:	 return (br->end1_up) ? "ã‚èŠK’i" : "‰º‚èŠK’i";
#endif
	}
...
#else
#define ADDNTOBUF(nam, var) { if (var) \
	Sprintf(eos(buf), "%s%s" nam, COMMA, seen_string((var), (nam))); }
#endif
...
boolean printdun;
{
...
#else
			Sprintf(buf, "%s:", jdgnnam(mptr->lev.dnum));
#endif
		else
#ifndef JP
...
#else
			Sprintf(buf, "%s: %dŠK-%dŠK‚Ü‚Å’TõÏ‚Ý", 
				jdgnnam(mptr->lev.dnum),
				depthstart, depthstart + 
				dungeons[mptr->lev.dnum].dunlev_ureached - 1);
#endif
	...
	if (INTEREST(mptr->feat)) {
		...
		if (mptr->feat.nshop > 1)
			ADDNTOBUF(E_J("shop", "“X"), mptr->feat.nshop)
		else if (mptr->feat.nshop == 1)
			Sprintf(eos(buf), "%s%s", COMMA, 
				E_J(an(shop_string(mptr->feat.shoptype)),
		...
		if (!mptr->feat.ntemple || mptr->feat.ntemple != mptr->feat.naltar)
			ADDNTOBUF(E_J("altar", "Õ’d"), mptr->feat.naltar)
		else
			ADDNTOBUF(E_J("temple","Ž›‰@"), mptr->feat.ntemple)

		/* only print out altar's god if they are all to your god */
		if (Amask2align(Msa2amask(mptr->feat.msalign)) == u.ualign.type)
			Sprintf(eos(buf), E_J(" to %s", "(%s)"), align_gname(u.ualign.type));
	...
	if (mptr->br) {
		 ...
		 */
		if (mptr->br->end1_up && !In_endgame(&(mptr->br->end2))) {
		    Sprintf(buf, PREFIX "%s(%dŠK)‚Ö‚Ì%s",
			    jdgnnam(mptr->br->end2.dnum),
			    depth(&(mptr->br->end2)),
			    br_string2(mptr->br)); 
		} else {
		    Sprintf(buf, PREFIX "%s‚Ö‚Ì%s",
			    jdgnnam(mptr->br->end2.dnum),
			    br_string2(mptr->br)); 
		}
#endif
