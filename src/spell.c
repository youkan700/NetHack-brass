...
throwspell()
{
	...
	cc.x = u.ux;
	cc.y = u.uy;
	mark_goodpos(&cc, 26, GOODPOS_CANSEE|GOODPOS_COULDSEE|GOODPOS_NOWALL);
	if (getpos(&cc, TRUE, E_J("the desired position","目標地点")) < 0)
	    return 0;	/* user pressed ESC */
	/* The number of moves from hero to where the spell drops.*/
	if (dist2(u.ux, u.uy, cc.x, cc.y) > 26) {
...
struct obj *weapon;
{
	...
	know_enchantment_worker(weapon, win);
...
winid win;
{
	...
	if (is_launcher(weapon)) {
...
#else
		const char *ammonam[4] = {"矢を射る", "ボルトを撃つ", "石を投擲する", "弾丸を発砲する"};
#endif /*JP*/
		...
		Sprintf(buf, E_J("Damage to small foes: %dd%d",
				 "小型の敵へのダメージ: %dd%d"), snum, sdie);
		if (snum2) Sprintf(eos(buf), "+%dd%d", snum2, sdie2);
		if (sdbon) Sprintf(eos(buf), "+%d", sdbon);
		putstr(win, 0, buf);
		Sprintf(buf, E_J("Damage to large foes: %dd%d",
				 "大型の敵へのダメージ: %dd%d"), lnum, ldie);
	...
	/* add artifact extra damage desc */
