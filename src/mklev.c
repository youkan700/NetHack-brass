...
STATIC_OVL void FDECL (remove_thinwalls, (SCHAR_P));
...
skip0:
	...
	for(croom = rooms; croom->hx > 0; croom++) {
		...
		if(u.uhave.amulet || !rn2(3)) {
		    ...
		    if (tmonst && tmonst->mnum == PM_GIANT_SPIDER &&
			    !occupied(x, y))
...
/*
    ...
    this should be called before wallify_map()
 */
STATIC_OVL void
remove_thinwalls(totyp)
schar totyp;
{
	int i, j;
	uchar wm;
