...
/*	Rule 5: You aren't yet acceptable - or are you? */
	  ...
	  if(not_capable()) {
		...
		if (!flooreffects(otmp, u.ux, u.uy, E_J("fall","落ちた"))) {
		...
		(void) hold_another_object(otmp, "Oops!  %s out of your grasp!",
					       ...
					       (const char *)0);
#else
		(void) hold_another_object(otmp, "おっと！ %sがあなたの手から滑り落ちた！",
...
prisoner_speaks (mtmp)
	register struct monst *mtmp;
{
	if (mtmp->mnum == PM_PRISONER &&
			(mtmp->mstrategy & STRAT_WAITMASK)) {
	    /* Awaken the prisoner */
	    if (canseemon(mtmp))
	    	pline(E_J("%s speaks:","%sは言った:"), Monnam(mtmp));
	    verbalize(E_J("I'm finally free!","ついに自由になれた！"));
	    mtmp->mstrategy &= ~STRAT_WAITMASK;
	    mtmp->mpeaceful = 1;
