...
int shotlimit;
{
	...
	if (obj->otyp == BOOMERANG) {
	    ...
	    struct monst *mtmp;
	    if (!getdir_or_pos(0, GETPOS_MONTGT, (char *)0, E_J("aim","狙う対象"))) {
...
skip_getdir:
	...
	/* Tech: Flurry */
	if (aml && obj->oclass == WEAPON_CLASS &&
	    (objects[obj->otyp].oc_wprop & WP_SUBTYPE) == WP_ARROW &&
	    tech_inuse(T_FLURRY))
	    multishot++; /* Let'em rip! */
	...
	if (tech_inuse(T_OFUDA)) {
	    You(E_J("throw %s.","%sに霊力を込めて投げた。"), xname(obj));
	}
...
dovfire()
{
	...
	if (uwep && is_gun(uwep)) {
	    ...
	    if (u.twoweap && is_gun(uswapwep)) {
		/* if 1st gun killed the target, stop firing 2nd gun */
		if (autotarget()) {
...
#else
		pline("%sは%sを受け止め、置いた。", Monnam(mon), xname(obj));
#endif /*JP*/
		return 0;
	    } else {
#ifndef JP
...
boolean quick;	/* u.dx/u.dy has been already set */
{
	...
	if (!quick) {
	    if (!getdir_or_pos(0, GETPOS_MONTGT, (char *)0, E_J("shoot","撃つ目標")))
		return 0;
	    if ((mtmp = m_at(u.ux+u.dx, u.uy+u.dy)) != 0 &&
		(mtmp->mx == (u.ux+u.dx) && mtmp->my == (u.uy+u.dy)))
		u.ulasttgt = mtmp;
	...
	if (!u.dx && !u.dy && !u.dz) {
		You(E_J("shoot yourself!","銃で自分を撃った！"));
		losehp(d(10,10), "銃で自殺した", NO_KILLER_PREFIX);
		goto xit;
	}
