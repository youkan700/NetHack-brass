...
STATIC_DCL int FDECL(shk_recapture, (struct monst *));
...
register boolean withbill;
{
	...
	for (; shkp; shkp = shkp->nmon) {
	    ...
	    if (shkp->isshk && (eshkp = ESHK(shkp)) != 0 &&
		(eshkp->billct || !withbill)) break;
	}

	if (shkp) {
	    if (NOTANGRY(shkp)) {
		if (eshkp->surcharge) pacify_shk(shkp);
	    } else {
		if (!eshkp->surcharge) rile_shk(shkp);
	    }
	}
	return(shkp);
}
...
register boolean zero_out;
{
	...
	if (on_level(&(eshkp->shoplevel), &u.uz))
	    rooms[eshkp->shoproom - ROOMOFFSET].resident =
...
register struct monst *mtmp, *mtmp2;
{
	struct eshk *eshkp;
	eshkp = ESHK(mtmp2);
	rooms[eshkp->shoproom - ROOMOFFSET].resident = mtmp2;
...
shopper_financial_report()
{
	...
	if (this_shkp && (eshkp = ESHK(this_shkp)) != 0 &&
	    !(eshkp->credit || shop_debt(eshkp))) {
...
#ifdef FIRSTAID
	if (obj->otyp == BANDAGE) bandage_disappears(obj);
#endif
	if (Has_contents(obj)) delete_contents(obj);
	if (obj->omon) monfree(obj->omon);
	if (obj->oextra) del_xdat_obj(obj, 0);
...
{
	struct eshk *eshkp = ESHK(shkp);
	register xchar x = eshkp->shk.x, y = eshkp->shk.y;
...
dopay()
{
	...
	for (shkp = next_shkp(fmon, FALSE);
	    ...
	    eshkp = ESHK(shkp);
	    if (eshkp->following) angrysk++;
	    if (canspotmon(shkp)) seensk++;
	    if (inhishop(shkp) && (*u.ushops == eshkp->shoproom))
		resident = shkp;
	}
...
boolean itemize;
{
	...
	if(itemize && u.ugold + eshkp->credit == 0L){
	...
	if (buy == PAY_BUY && u.ugold + eshkp->credit < ltmp) {
#ifndef JP
	    You("don't%s have gold%s enough to pay for %s.",
		stashed_gold ? " seem to" : "",
		(eshkp->credit > 0L) ? " or credit" : "",
		doname(obj));
#else
	    You("%sを買えるだけの金%s持っていない%s。",
		doname(obj),
		(eshkp->credit > 0L) ? "もクレジットも" : "を",
		stashed_gold ? "ようだ" : "");
#endif /*JP*/
...
register boolean dummy;
{
	...
	if (eshkp->billct == BILLSZ) {
		...
		if (otmp) You(E_J("got %s for free!","%sをただで手に入れた！"), doname(otmp));
	}
...
register struct obj *obj, *otmp;
{
	...
	eshkp = ESHK(shkp);
	if(eshkp->billct == BILLSZ) otmp->unpaid = 0;
	else {
		tmp = bp->price;
		bp = &(eshkp->bill_p[eshkp->billct]);
		bp->bo_id = otmp->o_id;
...
register boolean peaceful, silent;
{
	...
	eshkp = ESHK(shkp);
	if(peaceful) {
	    boolean credit_use = !!eshkp->credit;
...
xchar x, y;
{
	...
	"最近略奪を受けた当店に対し、商品在庫の補充にご協力いただき感謝いたします。"));
		subfrombill(obj, shkp);
		return;
	}
	...
	if (saw_walls) {
		...
		pline(E_J("Suddenly, the floor damage is gone!",
	        ...
	        pline(E_J("Suddenly, the trap is removed from the floor!",
			  "突然、罠が床から排除された！"));
	    else if (inside_shop(u.ux, u.uy) == eshkp->shoproom)
...
#define	GDIST(x,y)	(dist2(x,y,gx,gy))
		...
		if (!satdoor && shk_recapture(shkp)) return 1;
		if (Invis
#ifdef STEED
			|| u.usteed
#endif
...
# endif
	...
	if((um_dist(x, y, 1) && !uinshp) || cant_mollify ||
	   (u.ugold + eshkp->credit) < cost_of_damage
				|| !rn2(50)) {
...
getcad:
	...
	if(yn(qbuf) != 'n') {
		...
		verbalize(E_J("Oh, yes!  You'll pay!","いいや、払ってもらう！"));
		hot_pursuit(shkp);
		adjalign(-sgn(u.ualign.type));
	}
}
...
struct monst *shkp;
{
	...
	Sprintf(price, "%ld %s%s", cost, currency(cost),
		otmp->quan > 1L ? " each" : "");
#else
	Sprintf(price, "%s%ld %s", otmp->quan > 1L ? "一つ" : "",
		cost, currency(cost));
#endif /*JP*/
    }
    return price;
...
boolean altusage;
{
	...
	if (otmp->oclass == SPBOOK_CLASS) {
	    ...
	    arg1 = rn2(2) ? E_J("This is no free library, cad!  ",
				"ここは図書館じゃないぞ、おい！ ") : "";
	    arg2 = eshkp->debit > 0L ? E_J(" an additional","さらに") : "";
	} else if (otmp->otyp == POT_OIL) {
...
register xchar x, y;
{
	...
	eshkp = ESHK(shkp);
	if(eshkp->shd.x != u.ux || eshkp->shd.y != u.uy)
		return(FALSE);
	...
	switch(c) {
		...
		if(obj->oeroded || obj->oeroded2) {
			...
			verbalize(E_J("That's already been protected!",
				      "これはもう加工済みですよ！"));
			break;
		} else if (!is_damageable(obj) && obj->otyp != CRYSKNIFE) {
...
/* Remember if a pet is trying to steal a shop item */
void
...
struct obj *obj;
{
	...
	if (obj->otyp == CORPSE &&
	    touch_petrifies(&mons[obj->corpsenm])) return;

	shkp = shop_keeper(*in_rooms(mtmp->mx, mtmp->my, SHOPBASE));
...
struct monst *shkp;
{
	...
	for (i=0; i<8; i++) {
	    ...
	    /* pet is found... */
	    for (j=0; j<MSTOLENSZ; j++) {
		if (!ep->mstolen[j].m_id) continue;
		if (mtmp->m_id == ep->mstolen[j].m_id) {
		    /* a remembered thief is found */
		    if (ep->mstolen[j].o_id) {
			for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
			    if (otmp->o_id == ep->mstolen[j].o_id) {
				vis = cansee(shkp->mx,shkp->my) && cansee(mtmp->mx,mtmp->my) &&
				      (canspotmon(shkp) || canspotmon(mtmp));
				mstealm(shkp, mtmp, otmp, vis);
				for (otmp = shkp->minvent; otmp; otmp = otmp->nobj) {
				    if (otmp->o_id == ep->mstolen[j].o_id &&
					otmp->oclass != COIN_CLASS) {
					obj_extract_self(otmp);
					mdrop_obj(shkp, otmp, vis);
					ep->mstolen[j].m_id = 0;
					ep->mstolen[j].o_id = 0;
					break;
				    }
			...
			/* the thief picked up pieces of gold */
			if (!mtmp->mgold &&
			    cansee(shkp->mx,shkp->my) && cansee(mtmp->mx,mtmp->my) &&
			    (canspotmon(shkp) || canspotmon(mtmp))) {
			    char buf[BUFSZ];
			    Strcpy(buf, mon_nam(mtmp));
#ifndef JP
			    pline("%s takes %s back from %s!", Monnam(shkp),
				    mtmp->mgold == 1 ? "a gold piece" : "gold pieces",
				    buf);
#else
			    pline("%sは%sから金貨を取り戻した！", Monnam(shkp), buf);
#endif /*JP*/
			}
			/* take all gold: not accurate, but almost sufficient */
			shkp->mgold += mtmp->mgold;
			mtmp->mgold = 0;
			ep->mstolen[j].m_id = 0;
		    }
		}
