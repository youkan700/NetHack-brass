...
//			makeknown(otyp);
//		}
		if (!cancelled_don && levitation_off(0)) makeknown(otyp);
		break;
	case LOW_BOOTS:
	case IRON_SHOES:
	case HIGH_BOOTS:
...
Armor_on()
{
    ...
    if (is_heavy_armor(uarm)) encumber_msg();
    return 0;
}
...
//		}
		obj->spe = 0;
		if (levitation_on(oldprop, !obj->cursed)) {
		    makeknown(RIN_LEVITATION);
		}
		break;
...
//		if (!Levitation) makeknown(RIN_LEVITATION);
		if (levitation_off(obj)) makeknown(RIN_LEVITATION);
		break;
	case RIN_GAIN_STRENGTH:
		which = A_STR;
		goto adjust_attrib;
...
register struct obj *otmp;
{
	...
	if (otmp->otyp == GLASSES_OF_PHANTASMAGORIA &&
	    !oldprop && EHallucination) {
	    make_hallucinated(TIMEOUT, TRUE, 0L);
	    makeknown(GLASSES_OF_PHANTASMAGORIA);
	}
#endif
	if (changed) {
...
struct obj *ormv;
{
	...
	while (otyp = *p++) {
		...
		for (i=0; i<lpnum; i++) {
			...
			if (lpon && !nouw) {
				if (!oldprop && lprop == HALLUC_RES)
					make_hallucinated(FALSE, TRUE, mask);
				u.uprops[lprop].extrinsic |= mask;
				if (!oldprop && lprop == PROTECTION)
					u.uprotection += lpspe;
			} else {
				if (oldprop && lprop == HALLUC_RES)
					make_hallucinated(TRUE, TRUE, mask);
				u.uprops[lprop].extrinsic &= ~mask;
				if (oldprop && lprop == PROTECTION)
					u.uprotection -= lpspe;
			}
...
/* quickdowear() is putting on a suit without any delay or message.
   This is used for wearing the armor after removing it when the armor
   is polymorphed (ex. scales to DSM) */
void
quickdowear(otmp)
struct obj *otmp;
{
	...
	onfunc = ainfo->on_func;
	setworn(otmp, ainfo->mask);
	if (onfunc) (void) onfunc();
}
...
find_ac()
{
	...
	if(uarm)  uac -= (uarm->otyp  == ROBE_OF_PROTECTION ) ? ARM_BASE(uarm)  : ARM_BONUS(uarm);
