    ...
    register struct monst *magr,*mdef;
{
    ...
    if (magr->mnum == PM_GRID_BUG && magr->mx != mdef->mx
						&& magr->my != mdef->my)
	return(MM_MISS);
	...
	struct attack *mattk;
{
	...
	if (magr->mnum == PM_MEDUSA && mon_reflects(mdef, (char *)0)) {
	    if (canseemon(mdef))
...
struct monst *mdef;
{
	...
	for (otmp = mdef->minvent; otmp; otmp = otmp->nobj) {
		...
		if (can_teleport(mdef->data) && !mdef->mspec_used) {
			...
			You_hear(E_J("a zap inside from %s.","、%sの中で杖が振られる音を"),
				 mon_nam(magr));
		    otmp->spe--;
 ...
 physical:
		if (mattk->aatyp == AT_KICK && thick_skinned(pd)) {
...
#ifndef JP
				Sprintf(tmpbuf, "%s flesh", s_suffix(mon_nam(mdef)));
#else
				Sprintf(tmpbuf, "%sの身体", mon_nam(mdef));
#endif /*JP*/
			    else
				Strcpy(tmpbuf, mon_nam(mdef));
			    pline(E_J("The silver sears %s!",
				      "銀が%sを焼いた！"), tmpbuf);
			}
			/* poisoned weapon */
			if (otmp->opoisoned && !rn2(8)) {
				...
				if (!is_animobj(magr->data))
				    Sprintf(tmpbuf, E_J("%s ", "%s"), s_suffix(Monnam(magr)));
#ifndef JP
		...
		} else if (magr->mnum == PM_PURPLE_WORM &&
...
//		tmp = 0;
 ...
 post_stone:		if (mdef->mhp > 0) return 0;
		...
		break;
	    case AD_CURS:
		if (!night() && (magr->mnum == PM_GREMLIN)) break;
		if (!magr->mcan && !rn2(10)) {
		...
		if (obj) {
		    mstealm(magr, mdef, obj, vis);
		    if (mdef->mhp <= 0)
			return (MM_DEF_DIED | (grow_up(magr,mdef) ?
						0 : MM_AGR_DIED));
		    if (magr->data->mlet == S_NYMPH &&
			!tele_restrict(magr)) {
			if (vis) Strcpy(buf, Monnam(magr));
			(void) rloc(magr, FALSE);
			if (vis && !canspotmon(magr))
			    pline(E_J("%s suddenly disappears!",
				      "%sは突然消え失せた！"), buf);
	...
	/* animated object is destroyed by some means */
	if (is_animobj(mdef->data) && !mdef->mw) {
	    mondied(mdef);
	...
	/* animated object is destroyed by some means */
	if (is_animobj(magr->data) && !magr->mw) {
...
#ifndef JP
		pline("%s %ss %s from %s!", Monnam(magr),
			magr->isshk ? "recapture" : "steal",
			onambuf, mdefnambuf);
#else
		pline("%sは%sから%sを%s！", Monnam(magr),
			mdefnambuf, onambuf,
			magr->isshk ? "取り戻した" : "盗んだ");
#endif /*JP*/
