...
static int FDECL(check_race_genocide, (const char *));
static boolean FDECL(class_genocide_by_m2flag, (int));
static boolean FDECL(class_genocide_mindflayer, (int));
static boolean FDECL(class_genocide_hobbit, (int));
static boolean FDECL(class_genocide_by_sym, (int));
...
//					break;
	...
	case SCR_FIRE: {
		...
		explode(u.ux, u.uy, &zi, (2*(rn1(3, 3) + 2 * cval) + 1)/3, EXPL_FIERY);
		return(1);
	}
	case SCR_EARTH:
	    /* TODO: handle steeds */
...
boolean confused, helmet_protects, byu, skip_uswallow;
{
    ...
    if (dmg) losehp(dmg, E_J("scroll of earth","大地の巻物で"), E_J(KILLED_BY_AN, byu ? KILLED_BY_AN : KILLED_SUFFIX));
}
...
#define CLASS_GENO_NOTBYSYM  (MAXMCLASSES+1)

static boolean (*is_class_genocide_target)(int);
static int geno_class;
static unsigned long geno_m2flag;

static int
check_race_genocide(racenam)
const char *racenam;
{
	int i;
	static struct {
	...
	/* check if given race name matches to M2FLAG */
	for (i=0; racenam_to_M2[i].nam; i++) {
	    if (!strcmpi(racenam, racenam_to_M2[i].nam)) {
		geno_m2flag = racenam_to_M2[i].flag;
		is_class_genocide_target = class_genocide_by_m2flag;
		return CLASS_GENO_NOTBYSYM;
	    }
#ifdef JP
	    if (!strcmpi(racenam, racenam_to_M2[i].jnam)) {
		geno_m2flag = racenam_to_M2[i].flag;
		is_class_genocide_target = class_genocide_by_m2flag;
		return CLASS_GENO_NOTBYSYM;
	    }
	...
	/* check special case: mind flayer */
	if (strstri(racenam, "mind flayer")) {
	    is_class_genocide_target = class_genocide_mindflayer;
	    return CLASS_GENO_NOTBYSYM;
	}
#ifdef JP
	if (strstri(racenam, "マインドフレア")) {
	    is_class_genocide_target = class_genocide_mindflayer;
	    return CLASS_GENO_NOTBYSYM;
	}
#endif

	/* check special case: hobbit */
	if (!strcmpi(racenam, "hobbit")) {
	    is_class_genocide_target = class_genocide_hobbit;
	    return CLASS_GENO_NOTBYSYM;
	}
#ifdef JP
	if (!strcmpi(racenam, "ホビット")) {
	    is_class_genocide_target = class_genocide_hobbit;
	    return CLASS_GENO_NOTBYSYM;
	}
#endif

	/* nothing found: do genocide by monsym */
	return 0;
}

boolean class_genocide_by_m2flag(monnam)
int monnam;
{
	return ((mons[monnam].mflags2 & geno_m2flag) != 0);
}

boolean class_genocide_mindflayer(monnam)
int monnam;
{
	return (monnam == PM_MIND_FLAYER || monnam == PM_MASTER_MIND_FLAYER);
}

boolean class_genocide_hobbit(monnam)
int monnam;
{
	return (monnam == PM_HOBBIT);
}

boolean class_genocide_by_sym(monnam)
int monnam;
{
	return (mons[monnam].mlet == geno_class);
}
...
do_class_genocide()
{
	...
	unsigned long m2flag = 0;

	is_class_genocide_target = class_genocide_by_sym;
	geno_class = 0;
...
//			delayed_killer = killer;
			set_delayed_killer(POLYMORPH, killer_format, killer);
			killer = 0;
