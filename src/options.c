...
#ifdef CONFIRM_EX
	{ "confirm", E_J("your behavior to peaceful creatures","友好的な相手にぶつかったときの行動"), 7, SET_IN_GAME },
#endif
...
#ifdef SORTLOOT
	{ "sortloot", E_J("controls the sortloot patch","sortlootパッチの機能選択"), 4, SET_IN_GAME },
#endif
#ifdef MSDOS
...
#ifdef CONFIRM_EX
static NEARDATA const char *confirmtype[] = {
	"attack", "confirm", "stop"
};
#endif
...
boolean setinitial,setfromfile;
{
     ...
     * disclose, runmode, msg_window, menu_headings, number_pad and sortloot options.
#ifdef AUTOPICKUP_EXCEPTIONS
...
#endif
#ifdef CONFIRM_EX
    } else if (!strcmp("confirm", optname)) {
	    const char *confirm_name;
	    menu_item *confirm_pick = (menu_item *)0;
	    tmpwin = create_nhwindow(NHW_MENU);
	    start_menu(tmpwin);
	    for (i = 0; i < SIZE(confirmtype); i++) {
		confirm_name = confirmtype[i];
		any.a_char = *confirm_name;
		add_menu(tmpwin, NO_GLYPH, &any, *confirm_name, 0,
			ATR_NONE, confirm_name, MENU_UNSELECTED);
	    }
