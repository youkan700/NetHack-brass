...
//	if(!getdir((char *)0)) return(0);
	if(!getdir_or_pos(0, GETPOS_MONTGT, (char *)0, E_J("take a picture","写真を撮る対象"))) return 0;
...
//	if(!getdir((char *)0)) return 0;
	if(!getdir_or_pos(0, GETPOS_MONTGT, (char *)0, E_J("reflect","鏡に映す相手"))) return 0;
	if(obj->cursed && !rn2(2)) {
		if (!Blind)
...
int magic; /* 0=Physical, otherwise skill level */
{
	...
	cc.x = u.ux;
	cc.y = u.uy;
	range      = magic ? magic : 26;
	goodpos    = GOODPOS_CANSEE|GOODPOS_COULDSEE|GOODPOS_NOWALL;
	knightmove = (!magic && !(HJumping & ~INTRINSIC) && !EJumping);
	if (knightmove) {
	    goodpos |= GOODPOS_EXACTDIST;
	    range = 5;
	}
	mark_goodpos(&cc, range, goodpos);
	if (getpos(&cc, TRUE, E_J("the desired position","目標地点")) < 0)
		return 0;	/* user pressed ESC */
	if (knightmove && distu(cc.x, cc.y) != 5) {
...
void
bandage_disappears(obj)
struct obj *obj;
{
	if (obj == bandage.bandage) bandage.bandage = (struct obj *)0;
}
#endif /*FIRSTAID*/
...
#define ATTR_COUNT (A_MAX*3)	/* number of attribute points we might fix */
	...
	if (obj && obj->cursed) {
	    ...
	    case 0: make_sick(Sick ? Sick/3L + 1L : (long)rn1(ACURR(A_CON),20),
			E_J(xname(obj), "呪われたユニコーンの角の力で"), TRUE, SICK_NONVOMITABLE);
		    break;
...
#define attr_trouble(Y) trouble_list[trouble_count++] = attr2trbl(Y)
	...
	if (obj && (did_attr || did_prop) && !rn2(20)) {
		Your(E_J("unicorn horn disintegrates.",
			 "ユニコーンの角は崩れ去った。"));
		useup(obj);
	}
...
struct obj *otmp;
{
	...
	char buf[BUFSZ];
	const char *occutext = E_J("setting the trap","罠を仕掛けるの");
...
//    else if (ACURR(A_DEX) >= 14) proficient += (ACURR(A_DEX) - 14);
    ...
    } else if (mtmp && !is_animobj(mtmp->data)) {
	if (!canspotmons(mtmp) &&
	...
	if (otmp) {
		...
		case 3:
			...
			hitvalu = 8 + otmp->spe;
#ifdef JP
			bullet_killername(0, 0, "掴み取ろうとした武器が命中して死んだ");
#endif /*JP*/
			hitu = thitu(hitvalu,
	...
	struct obj *obj;
{
	...
	cc.x = u.ux;
	cc.y = u.uy;
	mark_goodpos(&cc, max_range, GOODPOS_COULDSEE|GOODPOS_NOWALL);
	...
	struct obj *obj;
{
	...
	cc.x = u.ux;
	cc.y = u.uy;
	mark_goodpos(&cc, max_range, GOODPOS_CANSEE|GOODPOS_COULDSEE|GOODPOS_NOWALL);
...
#else
		pline("%sを%sに装填することはできない。", xname(otmp), xname(obj));
#endif /*JP*/
		return (0);
	}
...
struct obj *obj;
{
	...
	if (!otmp) {
		You(E_J("don't have any guns.", "銃を持っていない。"));
		return 0;
	}
...
doapply()
{
	...
	case GLASSES_OF_KNOW_ENCHANTMENT:
	case GLASSES_OF_TRUE_SIGHT:
