...
boolean ghostly, frozen;
{
	...
	while(xl--) {
		...
		if (otmp->omon)
		    otmp->omon = restmonchn(fd, ghostly);
		if (otmp->oextra) {
		    otmp->oextra = restxdatchn(fd, ghostly);
...
boolean ghostly;
{
	...
	while(xl--) {
		...
		/* restore mtmp->data. if overriden by xdat, being fixed up later */
		set_mon_data(mtmp, mtmp->mnum, 0);
		if (ghostly) {
		...
		if (mtmp->mextra) {
			mtmp->mextra = restxdatchn(fd, ghostly);
...
unsigned int *stuckid, *steedid;	/* STEED */
{
	...
	restore_rndvault(fd);
	mread(fd, (genericptr_t) delayed_killers, sizeof delayed_killers);
...
boolean ghostly;
{
	...
	if (ghostly) {
	    ...
	    if ((tmpid = (unsigned *)get_xdat_obj(otmp, XDAT_M_ID)) != 0) {
		if (lookup_id_mapping(*tmpid, &nid))
		    *tmpid = nid;
	    }
	}
    }
