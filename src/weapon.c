...
struct skill_name {
	...
	{ P_DAGGER_GROUP,	E_J("dagger group",		"íZåï") },
	{ P_KNIFE_GROUP,	E_J("knife group",		"ÉiÉCÉt") },
	{ P_AXE_GROUP,		E_J("axe group",		"ïÄ") },
	{ P_SHORT_BLADE_GROUP,	E_J("short blade group",	"ÉVÉáÅ[ÉgÉ\Å[Éh") },
	{ P_SABER_GROUP,	E_J("saber group",		"ÉTÅ[ÉxÉã") },
	{ P_SABER_GROUP,	E_J("katana group",		"ÉJÉ^Éi") },
/*	{ P_BROAD_BLADE_GROUP,	"broad blade group" },*/
	{ P_LONG_BLADE_GROUP,	E_J("long blade group",		"ÉçÉìÉOÉ\Å[Éh") },
	{ P_CRUSHING_GROUP,	E_J("crushing group",		"ì›äÌ") },
	{ P_FLAIL_GROUP,	E_J("flail group",		"ÉtÉåÉCÉã") },
	{ P_STAFF_GROUP,	E_J("staff group",		"ñ_èp") },
	{ P_POLEARM_GROUP,	E_J("polearm group",		"í∑ïøïêäÌ") },
	{ P_SPEAR_GROUP,	E_J("spear group",		"ëÑ") },
	{ P_BOW_GROUP,		E_J("bow group",		"ã|") },
	{ P_FIREARM_GROUP,	E_J("firearm group",		"èeâŒäÌ") },
	{ P_THROWING_GROUP,	E_J("throwing group",		"ìäù±") },
	{ P_WHIP_GROUP,		E_J("whip group",		"ï⁄") },
	{ P_PICKAXE_GROUP,	E_J("pick-axe group",		"Ç¬ÇÈÇÕÇµ") },
	{ P_ATTACK_SPELL,	E_J("attack spell",		"çUåÇ") },
	{ P_HEALING_SPELL,	E_J("healing spell",		"é°ñ¸") },
	{ P_DIVINATION_SPELL,	E_J("divination spell",		"ó\ím") },
	{ P_ENCHANTMENT_SPELL,	E_J("enchantment spell",	"ï‚èï") },
	{ P_CLERIC_SPELL,	E_J("cleric spell",		"ëmóµ") },
	{ P_ESCAPE_SPELL,	E_J("escape spell",		"íEèo") },
	{ P_MATTER_SPELL,	E_J("matter spell",		"ï®éø") },
	{ P_BARE_HANDED_COMBAT,	E_J("bare handed combat",	"ëféËÇ≈ÇÃêÌì¨") },
	{ P_BARE_HANDED_COMBAT,	E_J("martial arts",		"É}Å[ÉVÉÉÉãÉAÅ[Éc") },
	{ P_TWO_WEAPON_COMBAT,	E_J("two weapon combat",	"ìÒìÅó¨") },
	{ P_ARMORED_COMBAT,	E_J("armored combat",		"èdëïêÌì¨") },
	{ P_SHIELD,		E_J("shield",			"èÇ") },
#ifdef STEED
	{ P_RIDING,		E_J("riding",			"èÊîn") },
#endif
	{ P_NONE,		"no skill" }
};
...
struct monst *mon;
{
	...
	if (is_flimsy(otmp) && thick_skinned(ptr))
		...
		tmp = 0;
	if (mon->mnum == PM_SHADE && mat != SILVER &&
...
static NEARDATA const int pwep[] = {
	HALBERD, VOULGE, LUCERN_HAMMER, MORNING_STAR, 
...
boolean polyspot;
{
	...
	if (!attacktype(mon->data, AT_WEAP)) {
		...
		if (cansee(mon->mx, mon->my)) {
		    pline(E_J("%s drops %s.","%sÇÕ%sÇóéÇ∆ÇµÇΩÅB"),
			  Monnam(mon), distant_name(obj, doname));
		    newsym(mon->mx, mon->my);
		}
		/* might be dropping object into water or lava */
		if (!flooreffects(obj, mon->mx, mon->my, E_J("drop","óéÇøÇΩ"))) {
...
register struct monst *mon;
{
	...
	/* animated object doesn't change its mon->mw */
	if (is_animobj(mon->data)) {
		mon->weapon_check = NO_WEAPON_WANTED;
		return 0;
	}
	...
	if (obj && obj != &zeroobj) {
...
#else
			    pline("ïêäÌÇ™%sÇ©ÇÁó£ÇÍÇ»Ç¢ÇΩÇﬂÅA%sÇÕ%sÇëïîıÇ≈Ç´Ç»Ç¢ÅB",
				  mon_hand, mon_nam(mon), xname(obj));
#endif /*JP*/
			} else {
			    pline(E_J("%s tries to wield %s.","%sÇÕ%sÇç\Ç¶ÇÊÇ§Ç∆ÇµÇΩÅB"),
		...
		mon->weapon_check = NEED_WEAPON;
		if (vis) {
		    pline(E_J("%s wields %s!","%sÇÕ%sÇëïîıÇµÇΩÅI"), Monnam(mon), doname(obj));
		...
		if (artifact_light(obj) && !obj->lamplit) {
			...
			pline("%s brilliantly in %s %s!",
			    ...
			    s_suffix(mon_nam(mon)), mbodypart(mon,HAND));
#else
			pline("%sÇÕ%sÇÃ%sÇ≈Ç‹Ç‘ÇµÇ≠ãPÇ´ÇÕÇ∂ÇﬂÇΩÅI",
			    xname(obj),  mon_nam(mon), mbodypart(mon,HAND));
#endif /*JP*/
		}
		obj->owornmask = W_WEP;
...
const static struct skill_range {
    ...
    { P_FIRST_H_TO_H, P_LAST_H_TO_H, E_J("Fighting Skills",	"êÌì¨ãZî\") },
    { P_FIRST_WEAPON, P_LAST_WEAPON, E_J("Weapon Skills",	"ïêäÌãZî\") },
    { P_FIRST_SPELL,  P_LAST_SPELL,  E_J("Spellcasting Skills",	"éÙï∂ârè•ãZî\") },
};
	...
	if (!to_advance) {
		...
		add_menu(win, NO_GLYPH, &any, 'w', 0, ATR_NONE,
				E_J("Show weapon capability",
				    "ïêäÌãZî\Ç…ëŒâûÇ∑ÇÈïêäÌÇå©ÇÈ"), MENU_UNSELECTED);
		any.a_int = 2+1;
		add_menu(win, NO_GLYPH, &any, 's', 0, ATR_NONE,
				E_J("Show spellcasting capability",
				    "ñÇñ@ãZî\Ç…ëŒâûÇ∑ÇÈéÙï∂Çå©ÇÈ"), MENU_UNSELECTED);
		any.a_int = 3+1;
		add_menu(win, NO_GLYPH, &any, 'c', 0, ATR_NONE,
				E_J("Show combat capability",
				    "êÌì¨ãZî\Çå©ÇÈ"), MENU_UNSELECTED);
	}
	Strcpy(buf, (to_advance) ? E_J("Pick a skill to advance:","è„íBÇ≥ÇπÇΩÇ¢ãZî\ÇëIÇÒÇ≈Ç≠ÇæÇ≥Ç¢:") :
				   E_J("Current skills:","åªç›ÇÃãZî\:"));
	end_menu(win, buf);
...
//	give_may_advance_msg(P_NONE);
	...
	if (newskill > P_MAX_SKILL(type)) {
		You(E_J("have become better at %s!",
			"%sÇÃãZî\Ç…ÇÊÇËèníBÇµÇΩÅI"), P_NAME(type));
...
#else
		Your("%sÇÃãZî\ÇÕ%så¸è„ÇµÇΩÅB", P_NAME(type),
		    (newskill == P_MAX_SKILL(type)) ? "ç≈ëÂå¿Ç‹Ç≈" : "Ç≥ÇÁÇ…");
#endif /*JP*/
	if (type == P_ARMORED_COMBAT) encumber_msg();
}
...
//	    P_ADVANCE(skill) = 0;
	...
	/* Set skill if you are wearing heavy armor */
	for (obj = invent; obj; obj = obj->nobj) {
	    if (obj->oclass != ARMOR_CLASS) continue;
	    if (is_heavy_armor(obj)) {
		P_SKILL(P_ARMORED_COMBAT) = P_BASIC;
	    } else if (objects[obj->otyp].oc_armcat == ARM_SHIELD) {
...
register struct obj *obj;
{
	...
	if (canseemon(mon) && !is_swallowed(mon))
	    ...
	    pline("%s in %s %s %s glowing.", The(xname(obj)),
		  ...
		  otense(obj, "stop"));
#else
	    pline("%sÇ™ç\Ç¶ÇƒÇ¢ÇΩ%sÇÕãPÇ≠ÇÃÇé~ÇﬂÇΩÅB",
		  mon_nam(mon), xname(obj));
#endif /*JP*/
    }
    if (MON_WEP(mon) == obj)
...
#else
	if (cnt)
	    Sprintf(buf, "íbòBÇ≈Ç´ÇÈ%sãZî\:", isweapon ? "ïêäÌ" : "éÙï∂");
	else
	    Sprintf(buf, "íbòBÇ≈Ç´ÇÈ%sãZî\ÇÕÇ†ÇËÇ‹ÇπÇÒ", isweapon ? "ïêäÌ" : "éÙï∂");
#endif /*JP*/
	end_menu(tmpwin, buf);
...
#else
	if (cnt)
	    Sprintf(buf, "íbòBÇ≈Ç´ÇÈêÌì¨ãZî\:");
	else
	    Sprintf(buf, "íbòBÇ≈Ç´ÇÈêÌì¨ãZî\ÇÕÇ†ÇËÇ‹ÇπÇÒ");
#endif /*JP*/
	end_menu(tmpwin, buf);
