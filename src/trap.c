...
STATIC_VAR const char * const A_Your[2] = { E_J("A",""), E_J("Your","") };
STATIC_VAR const char tower_of_flame[] = E_J("tower of flame","噴きあがる炎に焼かれて");
...
int msgtype;
{
	...
	if (victim == &youmonst) Sprintf(buf, "あなたの%sは", ostr);
	     ...
	     Sprintf(buf, "%sの%sは", Monnam(victim), ostr);
	...
	if (In_sokoban(&u.uz) && Can_fall_thru(&u.uz))
	   ...
	   || (Inhell && !u.uevent.invoked &&
	    ...
	    dont_fall = E_J("don't fall in.","落ちなかった。");
		   ...
		   (yn(E_J("Levitate to avoid it?","浮遊して回避しますか？")) == 'y')) {
...
#endif /*JP*/
			if (!iscl) {
			    if (yn(E_J("Levitate to avoid it?","浮遊して回避しますか？")) == 'y') {
				in_steed_dismounting = TRUE;
				start_levitation();
				in_steed_dismounting = FALSE;
			    } else
...
pitfall:
		    ...
		    if (u.usteed) {
			    ...
			    Sprintf(verbbuf, E_J("and %s fall","と%sは落とし穴に落ちた"),
				x_monnam(u.usteed,
				    u.usteed->has_name ? ARTICLE_NONE : ARTICLE_THE,
				    (char *)0, SUPPRESS_SADDLE, FALSE));
			else
			    Sprintf(verbbuf,E_J("lead %s","は%sを落とし穴に導いた"),
				x_monnam(u.usteed,
					 u.usteed->has_name ? ARTICLE_NONE : ARTICLE_THE,
		...
		losehp(rnd(16), E_J("land mine","地雷を踏んで"), KILLED_BY_AN);
	...
	while(dist-- > 0 && !used_up) {
		...
		if (style == ROLL) {
			...
			case TRAPDOOR:
			    ...
			    if (flooreffects(singleobj, x2, y2, E_J("fall","落ちた")))
				used_up = TRUE;
		    ...
		    if (flooreffects(singleobj, bhitpos.x, bhitpos.y, E_J("fall","落ちた"))) {
			used_up = TRUE;
			break;
		    }
		    if (otyp == BOULDER &&
...
register struct monst *mtmp;
{
	...
	if (!trap) {
		...
		if (sobj_at(BOULDER, mtmp->mx, mtmp->my) &&
			...
			mtmp->mtrapped = 0;
			if (canseemon(mtmp))
			    pline(E_J("%s pulls free...", "%sは抜け出した…。"),
				  Monnam(mtmp));
...
int x, y;
{
	...
	if ((t = t_at(x, y)) &&
		...
		obj_extract_self(otmp);
		(void) flooreffects(otmp, x, y, E_J("settle","納まった"));
	}
}
 ...
 */
	...
	if ((box && !carried(box)) ? is_pool(box->ox, box->oy) : Underwater) {
	    ...
	    if (Fire_resistance) You("傷つかなかった。");
	    else losehp(rnd(3), "熱水をかぶって", KILLED_BY);
#endif /*JP*/
	    return;
	}
#ifndef JP
...
drown()
{
 ...
 crawl:
	...
	killer = (levl[u.ux][u.uy].typ == POOL || Is_medusa_level(&u.uz)) ?
	    E_J("pool of water","水場に落ちて") : E_J("moat","堀に落ちて");
	done(DROWNING);
...
wiz_select_chest_trap()
{
	...
	for (i=1; i<=BOXTRAP_MAXNUM; i++) {
	    any.a_int = i;
	    Sprintf(buf, E_J("%s trap", "%s罠"), chest_trap_infotbl[i].nam);
