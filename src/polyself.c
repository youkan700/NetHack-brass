...
const char *fmt, *arg;
{
	...
	if ((mvitals[urole.malenum].mvflags & G_GENOD) ||
	    ...
	    killer = get_delayed_killer(POLYMORPH, &killer_format);
...
#else
	    	Sprintf(buf, "%sに乗って", JMONNAM(monsndx(u.usteed->data)));
#endif /*JP*/
	    	instapetrify(buf);
 	    }
	...
	for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
		...
		if (Invis && !perceives(mtmp->data))
		  ...
		  && mtmp->mtame) {
			...
			if (dmg && !DEADMONSTER(mtmp)) mlosehp(mtmp, dmg);
		     ...
		     */
		    if (!DEADMONSTER(mtmp) &&
			  (mtmp->mnum == PM_FLOATING_EYE) && !mtmp->mcan) {
			if (!Free_action) {
...
domindblast()
{
	...
	if (u.uen < 10) {
	    You(E_J("concentrate but lack the energy to maintain doing so.",
		    "精神を集中させようとしたが、エネルギー不足で持続できなかった。"));
	    return(0);
	}
	u.uen -= 10;
	flags.botl = 1;
	...
	*fish_parts[] = { "ひれ", "目", "顔", "ひれの先",
	...
	if (part == HAND || part == HANDED) {	/* some special cases */
	    ...
	    if (humanoid(mptr) && attacktype(mptr, AT_CLAW) &&
		    ...
		    mndx != PM_STONE_GOLEM &&
		    mndx != PM_INCUBUS && mndx != PM_SUCCUBUS)
		return part == HAND ? "claw" : "clawed";
	}
	if ((mndx == PM_MUMAK || mndx == PM_MASTODON) &&
		part == NOSE)
	    return "trunk";
#endif /*JP*/
	if (mndx == PM_SHARK && part == HAIR)
	    return E_J("skin","鮫肌");	/* sharks don't have scales */
	if (mndx == PM_JELLYFISH && (part == ARM || part == FINGER ||
	...
	if (mptr->mlet == S_EEL && mndx != PM_JELLYFISH)
	...
	if (mptr->mlet == S_JELLY || mptr->mlet == S_PUDDING ||
		mptr->mlet == S_BLOB || mndx == PM_JELLYFISH)
