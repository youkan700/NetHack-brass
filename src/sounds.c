...
dosounds()
{
	...
	for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
	    if (!DEADMONSTER(mtmp) && mtmp->mnum == PM_ORACLE)
		break;
...
register struct monst *mtmp;
{
    ...
    if (is_silent(ptr)) {
#ifdef JP
	if (Hallucination && mtmp->mnum == PM_GHOUL)
	...
	case MS_WERE:
		...
		pline("%s throws back %s head and lets out a blood curdling %s!",
		      ...
		      mtmp->mnum == PM_HUMAN_WERERAT ? "shriek" : "howl");
#else
		pline("%sは大きくのけぞると、血も凍るような%sを上げた！",
		      Monnam(mtmp), mtmp->mnum == PM_HUMAN_WERERAT ? "金切り声の叫び" : "雄叫び");
#endif /*JP*/
	...
	case MS_BARK:
		...
		if (mtmp->mtame &&
		    ...
		    if (mtmp->mnum != PM_DINGO)	/* dingos do not actually bark */
	...
	case MS_SQEEK:
	    ...
	    break;
	case MS_SQAWK:
	    if (mtmp->mnum == PM_RAVEN && !mtmp->mpeaceful)
