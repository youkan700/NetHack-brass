...
register xchar x, y;
{
	struct epri *ep = EPRI(priest);
	if (!ep) return FALSE;
	return((boolean)((ep->shroom == *in_rooms(x, y, TEMPLE)) &&
	       on_level(&(ep->shrlevel), &u.uz)));
}
...
boolean sanctum;   /* is it the seat of the high priest? */
{
	...
	atyp = Amask2align(levl[sx][sy].altarmask);
	priest = makemon(&mons[sanctum ? PM_HIGH_PRIEST : PM_ALIGNED_PRIEST],
			 sx + 1, sy, NO_MM_FLAGS);
	if (priest) {
		setmaligntyp(priest, atyp);
		add_xdat_mon(priest, XDAT_EPRI, 0);
		ep = EPRI(priest);
		ep->shroom = (sroom - rooms) + ROOMOFFSET;
		ep->shrpos.x = sx;
		ep->shrpos.y = sy;
		assign_level(&(ep->shrlevel), lvl);
 ...
 *	- minions do not have ispriest but have isminion
...
char *pname;		/* caller-supplied output buffer */
{
	...
	if (mon->ispriest || mon->mnum == PM_ALIGNED_PRIEST) {
		if (!ep) {
			impossible("priestname: A priest doesn't have epri?");
			Strcat(pname, "illegal priest");
			return pname;
		}
#ifdef JP
		Strcat(pname, halu_gname(atyp));
		Strcat(pname, "の");
#endif /*JP*/
		/* use epri */
		if (!mon->ispriest && ep->renegade)
			Strcat(pname, E_J("renegade ","破戒"));
#ifndef JP
		if (mon->mnum == PM_HIGH_PRIEST)
			Strcat(pname, "high ");
		if (Hallucination)
		...
		if (Hallucination) {
			Strcat(pname, "無能");
			Strcat(pname, (mon->mnum == PM_HIGH_PRIEST) ?
				      "大臣" : "官僚");
		} else if (mon->mnum == PM_HIGH_PRIEST)
	...
	/* minion */
#ifndef JP
	if (mon->mtame && mon->mnum == PM_ANGEL)
...
struct monst *priest;
{
	return((boolean)(u.ualign.type == getmaligntyp(priest)));
}
...
struct monst *pri;
{
	...
	ep = EPRI(pri);
	if (!ep) return(FALSE);
	lev = &levl[ep->shrpos.x][ep->shrpos.y];
	if (!IS_ALTAR(lev->typ) || !(lev->altarmask & AM_SHRINE))
		return(FALSE);
	return((boolean)(getmaligntyp(pri) == Amask2align(lev->altarmask & ~AM_SHRINE)));
}
...
register int roomno;
{
	...
	if(!temple_occupied(u.urooms0)) {
		...
		sanctum = (priest->mnum == PM_HIGH_PRIEST &&
...
boolean peaceful;
{
	...
	if (ptr->mnum != PM_ALIGNED_PRIEST && ptr->mnum != PM_ANGEL)
		return((struct monst *)0);
	...
	setmaligntyp(roamer, alignment);
	if (ptr->mnum == PM_ALIGNED_PRIEST) {
		add_xdat_mon(roamer, XDAT_EPRI, 0);
		ep = EPRI(roamer);
		ep->renegade = (coaligned && !peaceful);
	}
...
register struct monst *roamer;
{
	if(!(roamer->isminion && (roamer->mnum == PM_ALIGNED_PRIEST ||
				  roamer->mnum == PM_ANGEL)))
	        return;

	if(getmaligntyp(roamer) != u.ualign.type) {
	    roamer->mtame = 0;
