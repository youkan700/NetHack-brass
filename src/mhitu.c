...
/*	Special demon handling code */
	if(!mtmp->cham && is_demon(mdat) && !range2
	   && mtmp->mnum != PM_BALROG
	   && mtmp->mnum != PM_SUCCUBUS
	   && mtmp->mnum != PM_INCUBUS)
...
#else
				pline("%s%sが現れた！", from_nowhere, genericwere);
#endif /*JP*/
			    }
	...
	switch(mattk->adtyp) {
		...
		if (mattk->aatyp == AT_HUGS && !sticks(youmonst.data)) {
			...
			if ((otmp->oartifact &&
			    ...
			    nomsg = TRUE;
			    if (dmg > FATAL_DAMAGE_MODIFIER) instadeath = TRUE;
			}
			/* silver damage message */
			if (get_material(otmp) == SILVER &&
			    hates_silver(youmonst.data)) {
#ifndef JP
			    pline("The silver sears you%s!",
				  !noncorporeal(youmonst.data) ? "r flesh" : "");
#else
			    pline("銀があなた%sを焼いた！",
				  !noncorporeal(youmonst.data) ? "の身体" : "");
#endif /*JP*/
			    nomsg = TRUE;
			}
			/* poisoned weapon */
			if (otmp->opoisoned && !rn2(8)) {
			    int nopoison;
			    char tmpbuf[BUFSZ];
			    tmpbuf[0] = 0;
			    nopoison = (otmp->oartifact == ART_GRIMTOOTH) ? 1000 :
				       (objects[otmp->otyp].oc_merge) ? (10 - (otmp->owt/10)) : 100;
			    if (nopoison < 2) nopoison = 2;
			    hitmsg(mtmp, mattk); /* hitmsg should be printed before poison effect */
			    nomsg = TRUE;
#ifndef JP
			    Strcpy(tmpbuf, is_animobj(mtmp->data) ? xname(otmp) : mdat->mname);
#else
			    Sprintf(tmpbuf, "%sに殺された", mon_nam(mtmp));
#endif /*JP*/
			    poisoned((char *)0, A_STR, tmpbuf, 30);
			    if (otmp && !rn2(nopoison)) {
				otmp->opoisoned = FALSE;
				if (!is_animobj(mtmp->data))
				    Sprintf(tmpbuf, E_J("%s ", "%s"), s_suffix(Monnam(mtmp)));
#ifndef JP
				pline("%s%s %s no longer poisoned.",
				      tmpbuf, xname(otmp), otense(otmp, "are"));
#else
				pline("%s%sの毒は消え落ちた。", tmpbuf, xname(otmp));
#endif /*JP*/
			    }
			}
			/* stunning blow by a crushing weapon */
		...
		break;
	    case AD_DISE:
		if (uncancelled || mtmp->mnum == PM_PESTILENCE) {
		    hitmsg(mtmp, mattk);
...
dopois:
 ...
 do_stone:
				...
				set_delayed_killer(STONED, killer_format, delayed_killer);
#else
				Sprintf(dkiller_buf, "%sの攻撃で", JMONNAM(monsndx(mtmp->data)));
//				delayed_killer = dkiller_buf;
//				killer_format = KILLED_BY;
				set_delayed_killer(STONED, KILLED_BY, dkiller_buf);
#endif /*JP*/
				return(1);
				/* You("turn to stone..."); */
				/* done_in_by(mtmp); */
	    ...
	    case AD_CURS:
		hitmsg(mtmp, mattk);
		if(!night() && mtmp->mnum == PM_GREMLIN) break;
		if(!mtmp->mcan && !rn2(10)) {
		    if (flags.soundok) {
...
//		    delayed_killer = E_J(mtmp->data->mname, dkiller_buf);
		    set_delayed_killer(SLIMED, KILLED_BY_AN, E_J(mtmp->data->mname, dkiller_buf));
		} else
		    pline(E_J("Yuck!","うえっ！"));
		break;
 ...
 *	   2 if wrong gender for nymph */
{
	...
	if(pagr->mlet != S_NYMPH
		&& ((magr->mnum != PM_INCUBUS && magr->mnum != PM_SUCCUBUS)
#ifdef SEDUCE
		    || (mattk && mattk->adtyp != AD_SSEX)
#endif
		   ))
...
register struct monst *mon;
{
	...
	boolean fem = (mon->mnum == PM_SUCCUBUS); /* otherwise incubus */
	char qbuf[QBUFSZ];
...
//pline("(parry:%d)", tmp);
	if (rn2(20) < tmp) {
		...
		switch (mattk->aatyp) {
		    ...
		    case AT_TUCH:
			hitmsg(magr, mattk);
			You(E_J("block the attack with your shield!","攻撃を盾で防いだ！"));
			parried = 1;
			break;
		    case AT_KICK:
		    case AT_BUTT:
			hitmsg(magr, mattk);
			You(E_J("parry the attack with your shield!","攻撃を盾で受け流した！"));
			parried = 1;
			break;
		    case AT_WEAP:
			if (!MON_WEP(magr))
		...
		switch (mattk->aatyp) {
...
#else
			pline("%sは攻撃を盾で受け流した！", Monnam(mdef));
#endif
			parried = 1;
			break;
		    case AT_WEAP:
			if (magr == &youmonst) {
...
#else
					You("%sの盾を叩き割った！", mon_nam(mdef));
#endif
				    } else {
#ifndef JP
					You("attack %s and slice %s shield!", mon_nam(mdef), mhis(mdef));
#else
					You("%sの盾を真っ二つに斬り払った！", mon_nam(mdef));
#endif
				    }
				    m_useup(mdef, shield);
				    if (rn2(4)) {
			    ...
			    You(E_J("hit %s.","%sを攻撃した。"), mon_nam(mdef));
			}
#ifndef JP
			pline("%s parrys the attack with %s shield!", Monnam(mdef), mhis(mdef));
#else
		...
		if      (mattk->adtyp == AD_FIRE && is_flammable(shield))
		    rust_dmg(shield, E_J("the shield","盾"), 0, FALSE, mdef);
		else if (mattk->adtyp == AD_RUST && is_rustprone(shield))
		    rust_dmg(shield, E_J("the shield","盾"), 1, FALSE, mdef);
		else if (mattk->adtyp == AD_DCAY && is_rottable(shield))
		    rust_dmg(shield, E_J("the shield","盾"), 2, FALSE, mdef);
		else if (mattk->adtyp == AD_CORR && is_corrodeable(shield))
		    rust_dmg(shield, E_J("the shield","盾"), 3, FALSE, mdef);
	    }
	}
	return (parried);
}
