...
STATIC_DCL void NDECL(clear_goodpos);
static void
clear_goodpos()
{
    int x, y;
...
void
mark_goodpos(cc, rangelimit, flag)
coord *cc;
int rangelimit;
{
    int x, y, radius, dist;
    int range;
    schar ltyp;
    for (radius = 1; rangelimit > radius * radius; radius++);
    for (x = max(1, cc->x - radius); x <= min(COLNO-1, cc->x + radius); x++) {
	for (y = max(0, cc->y - radius); y <= min(ROWNO-1, cc->y + radius); y++) {
	    ltyp = levl[x][y].typ;
	    dist = dist2(x, y, cc->x, cc->y);
	    if (dist > rangelimit) continue;
	    if ((flag & GOODPOS_EXACTDIST) && dist != rangelimit) continue;
	    if ((flag & GOODPOS_CANSEE) && !cansee(x, y)) continue;
	    if ((flag & GOODPOS_COULDSEE) && !couldsee(x, y)) continue;
	    if ((flag & GOODPOS_NOWALL) && !MON_AT(x, y) &&
		(IS_ROCK(ltyp) || ltyp == DRAWBRIDGE_UP ||
		 (ltyp == DOOR && (levl[x][y].doormask & (D_NODOOR | D_BROKEN | D_ISOPEN) == 0)))) continue;
	    levl[x][y].mark = 1;
	    newsym(x, y);
	}
    }
...
const char *name;
{
	...
	if (mtmp->has_name) del_xdat_mon(mtmp, XDAT_NAME);
	add_xdat_mon(mtmp, XDAT_NAME, (genericptr_t) name);
...
do_mname()
{
	...
	(void) distant_monnam(mtmp, ARTICLE_THE, nam);
...
//	if ((aname = artifact_name(buf, &objtyp, 0)) != 0 && objtyp == obj->otyp)
//		Strcpy(buf, aname);
...
const char *name;
{
	...
	del_xdat_obj(obj, XDAT_NAME);
	if (*name) add_xdat_obj(obj, XDAT_NAME, (genericptr_t)name);
...
boolean called;
{
	...
	if (is_animobj(mdat)) {
	    if (mtmp->minvent && mtmp->mw && mtmp->minvent == mtmp->mw) {
		mn = xname(mtmp->mw);
	    }
	}
#ifdef JP
	/*  */
	if (mtmp->female && mtmp->mnum == PM_HIGH_PRIEST) mn = "女教皇";
#endif /*JP*/
	...
	if (mtmp->isshk && !do_hallu) {
	    ...
	    Strcat(buf, shkname(mtmp));
	    if (mtmp->mnum == PM_SHOPKEEPER && !do_invis)
		return buf;
	...
	if (name_at_start && (article == ARTICLE_YOUR || !has_adjectives)) {
	    if (mtmp->mnum == PM_WIZARD_OF_YENDOR)
		article = ARTICLE_THE;
	    else
		article = ARTICLE_NONE;
...
register struct monst *mtmp;
{
	return(x_monnam(mtmp, ARTICLE_NONE, (char *)0, 
		mtmp->has_name ? SUPPRESS_SADDLE : 0, TRUE));
}
...
{
	return(x_monnam(mtmp, ARTICLE_THE, (char *)0,
		mtmp->has_name ? (SUPPRESS_SADDLE|SUPPRESS_IT) :
		    SUPPRESS_IT, FALSE));
}
...
struct monst *mtmp;
{
	...
	suppression_flag = (mtmp->has_name
#ifdef STEED
...
char *outbuf;
{
    /* high priest(ess)'s identity is concealed on the Astral Plane,
       ...
       its own obfuscation) */
    if (mon->mnum == PM_HIGH_PRIEST && !Hallucination &&
...
static NEARDATA const char * const hcolors[] = {
	...
	"しょっぱい", "甘い", "酸っぱい", "苦い",
	"縞々の", "渦巻き模様の", "ぐるぐるの", "タータンチェックの", "碁盤目の", "アーガイル柄の",
...
static const char * const coynames[] = {
	...
	"キャニウス・シンケイシツ・レックス","ショクヨク・アリスギウス",
	"ショーモナ・シカエシス","キャニス・ラトランス"
#endif /*JP*/
};
