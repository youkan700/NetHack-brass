...
shuffle_tiles()
{
	...
	for (i = 0; i < NUM_OBJECTS; i++)
		tmp_tilemap[i] =
			glyph2tile[objects[i].oc_descr_idx + GLYPH_OBJ_OFF];/*FIXME*/

	for (i = 0; i < NUM_OBJECTS; i++)
		glyph2tile[i + GLYPH_OBJ_OFF] = tmp_tilemap[i];/*FIXME*/
}
#endif	/* USE_TILES */
...
dodiscovered()				/* free after Robert Viduya */
{
    ...
    putstr(tmpwin, 0, E_J("Discoveries","発見物一覧"));
    putstr(tmpwin, 0, "");
	...
	if (objects[uniq_objs[i]].oc_name_known) {
	    if (!dis++)
		putstr(tmpwin, iflags.menu_headings, E_J("Unique Items","特別な品物"));
	    Sprintf(buf, "  %s", E_J(OBJ_NAME(objects[uniq_objs[i]]),
				    JOBJ_NAME(objects[uniq_objs[i]])));
#ifdef JP
	    if (uniq_objs[i] == AMULET_OF_YENDOR) Strcpy(eos(buf), "魔除け");
	    else if (uniq_objs[i] == RIN_PORTAL) Strcpy(eos(buf), "指輪");
#endif /*JP*/
	    putstr(tmpwin, 0, buf);
	    ++ct;
	}
	...
	*s = '\0';
	...
	if (mvitals[i].mvflags & MV_KNOWS_EGG) {
	    if (eggcnt++ == 0)
		putstr(tmpwin, iflags.menu_headings, E_J("Eggs","卵"));
	    tmpegg.corpsenm = i;
