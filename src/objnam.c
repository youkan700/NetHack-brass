...
{
    return((boolean)(obj->dknown && obj->known &&
...
#endif
{
	...
	if ((obj->has_name || obj->oartifact) && obj->dknown) {
		...
		Sprintf(buf, "%sと名づけられた", named_obj_name(obj));
	}
#else /*JP*/
	if (obj_is_pname(obj))
	    goto nameit;
	...
	switch (obj->oclass) {
		...
		if (typ == STATUE) {
		    if (get_material(obj) == LIQUID &&
			can_be_frozen(&mons[obj->corpsenm])) { /* ice-bound monster */
			Sprintf(eos(buf), "氷漬けの%sの死体", JMONNAM(obj->corpsenm));
		    } else {
			Sprintf(eos(buf), "%sの", JMONNAM(obj->corpsenm));
...
register struct obj *obj;
{
	...
	if (obj->dknown) {
	    ...
	    if (obj->has_name) {
		int len = ONAMELTH(obj) - 1;
		if (!strncmp(bp, ONAME(obj), len) &&
		    !strncmp(bp + len, "と名づけられた", 14)) {
		    bp += len + 14;
		    nazuke = TRUE;
		}
	    }
	}
	if (obj->opoisoned && !strncmp(bp, "毒の塗られた", 12)) {
		bp += 12;
		ispoisoned = TRUE;
	}
#else /*JP*/
...
#ifdef JP
	if (nazuke && obj->otyp != CORPSE && obj->otyp != EGG)
	    Sprintf(eos(prefix), "%sと名づけられた", named_obj_name(obj));
#endif /*JP*/
...
plus:
	...
	case ARMOR_CLASS:
		if(obj->owornmask & W_ARMOR)
			Strcat(bp, (obj == uskin) ? E_J(" (embedded in your skin)"," (皮膚と同化している)") :
				E_J(" (being worn)"," (着用中)"));
		goto plus;
	case TOOL_CLASS:
	...
	case FOOD_CLASS:
		...
		if (obj->otyp == CORPSE) {
			...
			Sprintf(bp, "%sという名の%sの%s死体",
				named_obj_name(obj), JMONNAM(obj->corpsenm),
				obj->oeaten ? "食べかけの" : "");
		    } else {
		    ...
		    if (obj->has_name && obj->dknown)
			Sprintf(bp, "%sと名づけられた", named_obj_name(obj));
		    else
			*bp = 0;
...
struct obj *obj;
{
    ...
    if (!obj->oartifact) obj->has_name = 0;
...
boolean from_user;
{
	...
	cnt = spe = spesgn = typ = very = rechrg =
		...
		material = parenthesis_done = isdancing = istrapped =
#ifdef JP
		jmonnam =
#endif /*JP*/
...
#define SPINACH 2
	...
	ztoh(bp);
	/* こて対策 */
	if (!strcmpr(bp, "小手") || !strcmpr(bp, "籠手"))
	    strcpy(eos(bp) - 4, "篭手");
#endif /*JP*/
...
prefixes:
	for(;;) {
		...
		/* "unlabeled" and "blank" are synonymous */
			...
			contents = EMPTY;
		} else if(!strncmpi(bp, "dancing ", l=8)
#ifdef JP
			  || !strncmp(bp, "宙を舞う", l=8)
#endif /*JP*/
			 ) {
...
srch:
	...
	if(!oclass && actualn) {
	    ...
	    name = artifact_name(actualn, &objtyp, 0);
	    if(name) {
		typ = objtyp;
		goto typfnd;
	    }
	...
	if (wizard && from_user) {
		...
		for (trap = NO_TRAP+1; trap < TRAPNUM; trap++) {
			...
			tname = E_J(defsyms[trap_to_defsym(trap)].explanation,
				    defsyms[trap_to_defsym(trap)].eexplanation);
...
typfnd:
...
#ifdef WIZARD
		    if (wizard) {
			otmp->corpsenm = wiz_select_chest_trap();
		    } else
#endif
		    trap_chest(otmp);
		}
	...
	if (name) {
		...
		aname = artifact_name(name, &objtyp, &anum);
		if (aname && objtyp == otmp->otyp &&
		    !exist_artifact(anum))
			artifact_exists(otmp, aname, TRUE);
		else
	...
	if ((is_quest_artifact(otmp) ||
	     (otmp->oartifact && rn2(u.uconduct.wisharti) > 1))
#ifdef WIZARD
	    && !wizard
#endif
	    ) {
...
#else
	    pline("一瞬、あなたの%sに何かが現れた気がしたが、それは消え失せた！",
		  body_part(HAND));
#endif /*JP*/
	}
...
#ifdef WIZARD
	/* dancing weapon test */
	if (wizard && isdancing && otmp) {
		struct monst *mtmp;
		mtmp = makemon(&mons[PM_ANIMATED_OBJECT], u.ux, u.uy,
				MM_ADJACENTOK|MM_NOCOUNTBIRTH|NO_MINVENT);
		if (mtmp) {
			mpickobj(mtmp, otmp);
			mtmp->weapon_check = NO_WEAPON_WANTED;
			mtmp->mw = otmp;		/* force wield it */
