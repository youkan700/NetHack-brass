    ...
    char *buf, *monbuf;
{
	...
	pm = u.ustuck->data;
	       ...
	       (glyph_is_object(glyph) && (mtmp = m_at(x,y)) != 0 &&
	        is_animobj(mtmp->data)) ) {
	bhitpos.x = x;
	bhitpos.y = y;
	mtmp = m_at(x,y);
	if (mtmp != (struct monst *) 0) {
	    ...
	    if (mtmp->mnum == PM_COYOTE && accurate)
		name = coyotename(mtmp, monnambuf);
	    else if (is_animobj(mtmp->data)) {
		Strcpy(monnambuf, E_J("dancing ", "宙を舞う"));
		(void) distant_monnam(mtmp, ARTICLE_NONE, eos(monnambuf));
		name = monnambuf;
	    } else
	...
	if (!otmp || otmp->otyp != glyph_to_obj(glyph)) {
		...
		if (otmp->oclass == COIN_CLASS)
		    ...
		    int mat = (glyph & GLYPH_SUB_MASK) >> GLYPH_SUB_SHIFT;
		    if (mat <= MAX_MATERIAL) otmp->madeof = mat;
		    otmp->corpsenm = (glyph & GLYPH_MAIN_MASK);
		}
	...
	Strcpy(buf, def_warnsyms[(glyph & GLYPH_SUB_MASK) >> GLYPH_SUB_SHIFT].explanation);
...
/*	if (!getdir("^")) return 0;*/
	if (!getdir_or_pos(0, GETPOS_TRAP, "^", E_J("identify","識別する罠"))) return 0;
	x = u.ux + u.dx;
	y = u.uy + u.dy;
...
struct obj *otmp;
{
...
#else
	Sprintf(buf, "%sだ。", distant_name(otmp, xname));
#endif
    ...
    if ((otmp->oclass == WEAPON_CLASS || is_weptool(otmp)) && objects[otmp->otyp].oc_name_known) {
	putstr(datawin, 0, "");
	know_enchantment_worker(otmp, datawin);
    } else if (otmp->oclass == ARMOR_CLASS && 
               (otmp->oartifact == ART_FIST_OF_FURY || otmp->oartifact == ART_GAUNTLETS_OF_DEFENSE)) {
	know_enchantment_artifact_worker(otmp, datawin);
    }
disp:
...
int techid;
{
    ...
    if (techid < 0 || techid >= MAXTECH) {
	pline("Unexpected tech id: %d", techid);
	return;
    }
    ...
    ep = (char *)0;
    oindex = NUM_OBJECTS * 2 + NROFARTIFACTS + 1 + techid;
    if (dlb_fseek(fp, oindex * sizeof(long), SEEK_SET) < 0) {
	ep = "? Seek error on 'objdesc' file!";
	goto xit;
    }
    ...
    datawin = create_nhwindow(NHW_MENU);
    putstr(datawin, ATR_BOLD, E_J(tech_props[techid].name, jtech_names[techid]));
    putstr(datawin, 0, "");
