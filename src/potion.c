...
boolean talk;
{
	...
	if (xtime && !old) {
//		if (Free_action) {
//		    if (talk) You(E_J("momentaly stagger.","一瞬ふらついたが、踏みとどまった。"));
//		    xtime = 0;
//		} else
		if (talk) {
#ifdef STEED
...
register struct obj *obj;
{
	...
	if(!(mtmp = makemon(&mons[PM_DJINNI], u.ux, u.uy, NO_MM_FLAGS))){
		pline(E_J("It turns out to be empty.","中身は空だったようだ。"));
		return;
	}
