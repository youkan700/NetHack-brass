...
int attk;
{
	...
	if (is_animobj(mdef->data)) {
	    rust_dmg(mdef->mw, xname(mdef->mw), hurt, TRUE, mdef);
	    return;
	}
...
struct obj *wep;	/* uwep for attack(), null for kick_monster() */
{
	...
	if (iflags.confirm != 'a' && mtmp->mpeaceful
		...
		if (canspotmon(mtmp)) {
			...
			Norep(E_J("%s is on your way.",
				  "あなたの行く手に%sがいる。"),  Monnam(mtmp));
			flags.move = 0;
			return(TRUE);
		    }
#endif /*CONFIRM_EX*/
...
register struct monst *mtmp;
{
	...
	/* primary weapon */
	if (amain) {
	    tmp = find_roll_to_hit(mtmp, hitflags|(uwep ? HIT_MAINWEP : 0));
	    if (Upolyd)
		/* assume monsters don't have ranged attack */
		(void) hmonas(mtmp, tmp, hitflags|(uwep ? HIT_MAINWEP : 0));
	    else
		malive = hitum(mtmp, tmp, youmonst.data->mattk, hitflags|(uwep ? HIT_MAINWEP : 0));
	}
...
uchar hflg;
{
	...
	if (override_confirmation && wep &&
	    wep->oartifact == ART_STORMBRINGER && wep->blessed)
	    wep->blessed = 0;
...
uchar hflg;
{
	...
	for (reroll = u.uhitinc; reroll > 0 && !mhit; reroll--) {
if (wizard) pline("[reroll]");
	    mhit = (tmp > (dieroll = rnd(20)));
	}
...
int thrown;
{
	...
	anger_guards = (mon->mpeaceful &&
			    (mon->ispriest || mon->isshk ||
			     mon->mnum == PM_WATCHMAN ||
			     mon->mnum == PM_WATCH_CAPTAIN));
...
int thrown;
{
	...
	if(!obj) {	/* attack with bare hands */
		...
		if (/* if you strike with a bow... */
		    ...
		    if (mon->mnum == PM_SHADE && get_material(obj) != SILVER)
			tmp = 0;
		    else
			tmp = rnd(2);
		...
		mdat = mon->data;
		tmp = (mon->mnum == PM_SHADE) ? 0 : 1;
	    } else {
		if (mon->mnum == PM_SHADE && !shade_aware(obj)) {
		    tmp = 0;
...
#undef useup_eggs
		    ...
		    case SCISSORS:
			if (!thrown &&
			    (mon->mnum == PM_ROPE_GOLEM ||
			     mon->mnum == PM_PAPER_GOLEM)) {
...
//hpbar(mon,tmp);
	if (!already_killed) mlosehp(mon, tmp);
...
//	if (mon->mhp > mon->mhpmax) mon->mhp = mon->mhpmax;
	...
	if((mon->mnum == PM_BLACK_PUDDING || mon->mnum == PM_BROWN_PUDDING)
		   && obj && obj == uwep
...
struct attack *mattk;
{
	...
	if (is_animobj(mdef->data)) return;
...
//		}
		...
		if (mdef->mnum == PM_STRAW_GOLEM ||
		    mdef->mnum == PM_PAPER_GOLEM) {
		    if (!Blind)
...
//		}
		...
		break;
	    case AD_RUST:
		if (mdef->mnum == PM_IRON_GOLEM) {
			pline(E_J("%s falls to pieces!","%sはばらばらに崩れた！"), Monnam(mdef));
			xkilled(mdef,0);
		}
		hurtmarmor(mdef, AD_RUST);
		if (is_animobj(mdef->data) && !mdef->mw) xkilled(mdef,0);
//		tmp = 0;
		break;
	    case AD_CORR:
		...
		break;
	    case AD_DCAY:
		if (mdef->mnum == PM_WOOD_GOLEM ||
		    mdef->mnum == PM_LEATHER_GOLEM) {
		...
		if (notonhead || !has_head(mdef->data)) {
		    ...
		    tmp = 0;
		    if (!Unchanging && mdef->mnum == PM_GREEN_SLIME) {
			if (!Slimed) {
...
//		    killer_format = KILLED_BY_AN;
		    set_delayed_killer(STONED, KILLED_BY_AN, E_J(mdef->data->mname, dkiller_buf));
		}
		if (!vegan(mdef->data))
		    u.uconduct.unvegan++;
		...
		switch(mattk->adtyp) {
			...
			if (is_rider(mdef->data)) {
			 pline(E_J("Unfortunately, digesting any of it is fatal.",
			    ...
			    end_engulf();
			    Sprintf(msgbuf, E_J("unwisely tried to eat %s","愚かにも%sを食べようとした"),
				    E_J(mdef->data->mname, JMONNAM(monsndx(mdef->data))));
			    killer = msgbuf;
...
uchar hflg;
{
	...
	int	die;
	int     reroll;
	int	ranged = (hflg & HIT_DISTANT);

	for(i = 0; i < NATTK; i++) {

	    hflg &= ~(HIT_MAINWEP|HIT_SWAPWEP);
	    sum[i] = 0;
...
use_weapon:
	 ...
	 */
			if (ranged && (!uwep || !is_ranged(uwep))) continue;
			if (uwep) {
			...
			for (reroll = u.uhitinc; reroll > 0 && !dhit; reroll--) {
			    dhit = (tmp > (dieroll = rnd(20)));
			}
	...
	switch(ptr->mattk[i].adtyp) {
	  ...
	  case AD_STON:
		...
		if (protector == 0L ||		/* no protection */
		    ...
		    if (hflg & (HIT_MAINWEP|HIT_SWAPWEP))
			/* obj in hand was destroyed, hero didn't touch
