	...
	struct obj *otmp;
{
	...
	if (mtmp->mnum == PM_INCUBUS || mtmp->mnum == PM_SUCCUBUS) {
	...
	if (Confusion || Fumbling || Glib)
	    chance -= 20;
	else if (uarmg && objects[uarmg->otyp].oc_descr_idx == RIDING_GLOVES)
	    /* Bonus for wearing "riding" (but not fumbling) gloves */
	    chance += 10;
	else if (uarmf && objects[uarmg->otyp].oc_descr_idx == RIDING_BOOTS)
	    /* ... or for "riding boots" */
	    chance += 10;
	if (otmp->cursed)
	    chance -= 50;
	...
	boolean force;		/* Quietly force this animal */
{
	...
	if (Levitation && Lev_at_will) {
	    stopflying = E_J("stop levitating and ","そっと降下し、");
	    /* stop_levitation() calls float_down(), but we prefer
	       direct landing to the steed */
	    stop_ring_levitation(0);
	    HLevitation &= ~(TIMEOUT|I_SPECIAL);
	}
	/* Notify to player if s/he is no longer flying */
	if (Flying && !is_flying(mtmp)) {
	    stopflying = E_J("stop flying and ","飛行をやめ、");
	}
	...
	if (!force) {
	    ...
	    You(E_J("%smount %s.","%s%sに騎乗した。"), stopflying, mon_nam(mtmp));
	}
...
coord *cc;
{
	...
	for (i=0; i<8; i++) {
	    ...
	    j++;
	    j &= 7;
	    if (goodpos(x, y, mtmp, MM_IGNOREWATER)) {
		if (cc) {
		    cc->x = x;
		    cc->y = y;
		}
