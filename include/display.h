...
#define GLYPH_MAIN_MASK		0x00000FFF
#define GLYPH_TYPE_MASK		0x0000F000
#define GLYPH_SUB_MASK		0x00FF0000
#define GLYPH_SUB_SHIFT		16
#define GLYPH_ATTR_FRIEND	0x01000000
#define GLYPH_ATTR_DETECT	0x02000000
#define GLYPH_ATTR_RIDING	0x04000000
#define GLYPH_ATTR_GOODPOS	0x08000000

#define	GLYPH_TYPE_MON		0x00000000
#define	GLYPH_TYPE_BODY		0x00001000
#define	GLYPH_TYPE_STATUE	0x00002000
#define	GLYPH_TYPE_SWALLOW	0x00003000
#define	GLYPH_TYPE_OBJ		0x00004000
#define	GLYPH_TYPE_OTHERS	0x0000F000

#define GLYPH_CMAP_OFF		0
#define GLYPH_EXPLODE_OFF	((MAXPCHARS - MAXEXPCHARS) + GLYPH_CMAP_OFF)
#define GLYPH_ZAP_OFF		((MAXEXPCHARS * EXPL_MAX) + GLYPH_EXPLODE_OFF)
#define GLYPH_WARNING_OFF	((NUM_ZAP << 4) + GLYPH_ZAP_OFF)
#define GLYPH_INVIS_OFF		(1		+ GLYPH_WARNING_OFF)
#define GLYPH_CLOUD_OFF		(1		+ GLYPH_INVIS_OFF)
#define GLYPH_FIREBALL_OFF	(MAXCCHARS	+ GLYPH_CLOUD_OFF)
#define MAX_GLYPH		0x0FFFFFFF

#define NO_GLYPH MAX_GLYPH

#define GLYPH_INVISIBLE (GLYPH_INVIS_OFF|GLYPH_TYPE_OTHERS)

#define warning_to_glyph(mwarnlev)	(((mwarnlev)<<GLYPH_SUB_SHIFT)|GLYPH_WARNING_OFF|GLYPH_TYPE_OTHERS)
#define mon_to_glyph(mon)		((int) what_mon((mon)->mnum)|GLYPH_TYPE_MON)
#define detected_mon_to_glyph(mon)	(mon_to_glyph(mon)|GLYPH_ATTR_DETECT)
#define riding_mon_to_glyph(mon)	(mon_to_glyph(mon)|GLYPH_ATTR_RIDING)
#define pet_to_glyph(mon)		(mon_to_glyph(mon)|GLYPH_ATTR_FRIEND)
...
#define cmap_to_glyph(cmap_idx) ((int) (cmap_idx) + GLYPH_CMAP_OFF + GLYPH_TYPE_OTHERS)
#define explosion_to_glyph(expltype,idx)					\
		((((expltype) * MAXEXPCHARS) + ((idx) - S_explode1))		\
		 + GLYPH_EXPLODE_OFF + GLYPH_TYPE_OTHERS)
#define fireball_to_glyph(zaptype)	((int)(zaptype) + GLYPH_FIREBALL_OFF + GLYPH_TYPE_OTHERS)

#define trap_to_glyph(trap)	\
			cmap_to_glyph(trap_to_defsym(what_trap((trap)->ttyp)))
#define cloud_to_glyph(cloud)	\
	((cloud) - S_thinpcloud + GLYPH_CLOUD_OFF + GLYPH_TYPE_OTHERS)

/* Not affected by hallucination.  Gives a generic body for CORPSE (and STATUE) */
#define objnum_to_glyph(onum)		((int) (onum) + GLYPH_TYPE_OBJ)
#define monnum_to_glyph(mnum)		((int) (mnum) + GLYPH_TYPE_MON)
#define detected_monnum_to_glyph(mnum)	(monnum_to_glyph(mnum) | GLYPH_ATTR_DETECT)
#define petnum_to_glyph(mnum)		(monnum_to_glyph(mnum) | GLYPH_ATTR_FRIEND)
...
#define glyph_to_mon(glyph)						\
	(glyph_is_monster(glyph) ? ((glyph) & GLYPH_MAIN_MASK) :	\
	 glyph_is_statue(glyph) ?  ((glyph) & GLYPH_MAIN_MASK) :	\
	 NO_GLYPH)
#define glyph_to_obj(glyph)						\
	(glyph_is_body(glyph) ?		 CORPSE :			\
	 glyph_is_statue(glyph) ?	 STATUE :			\
	 glyph_is_normal_object(glyph) ? ((glyph) & GLYPH_MAIN_MASK) :	\
	 NO_GLYPH)
#define glyph_to_trap(glyph)						\
	(glyph_is_trap(glyph) ?						\
	 ((int) defsym_to_trap(((glyph) & GLYPH_MAIN_MASK) - GLYPH_CMAP_OFF)) :	\
	NO_GLYPH)
#define glyph_to_cmap(glyph)						\
	(glyph_is_cmap(glyph) ?						\
	 (((glyph) & GLYPH_MAIN_MASK) - GLYPH_CMAP_OFF) :		\
	 NO_GLYPH)
#define glyph_to_swallow(glyph)						\
	(glyph_is_swallow(glyph) ?					\
	 (((glyph) & GLYPH_SUB_MASK) >> GLYPH_SUB_SHIFT) :		\
	 0)
...
#define glyph_to_cloud(glyph)						\
	(glyph_is_cloud(glyph) ?					\
	 (((glyph) & GLYPH_MAIN_MASK) - GLYPH_CLOUD_OFF) :		\
	 NO_GLYPH)
...
#define glyph_is_monster(glyph)						\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_MON)
#define glyph_is_pet(glyph)						\
	(((glyph) & GLYPH_ATTR_FRIEND) != 0)
#define glyph_is_body(glyph)						\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_BODY)
#define glyph_is_riding_monster(glyph)					\
	(((glyph) & GLYPH_ATTR_RIDING) && glyph_is_monster(glyph))
#define glyph_is_detected_monster(glyph)				\
	(((glyph) & GLYPH_ATTR_DETECT) && glyph_is_monster(glyph))
#define glyph_is_invisible(glyph)					\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OTHERS &&		\
	 ((glyph) & GLYPH_MAIN_MASK) == GLYPH_INVIS_OFF)
#define glyph_is_statue(glyph)						\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_STATUE)
#define glyph_is_normal_object(glyph)					\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OBJ)
#define glyph_is_swallow(glyph)						\
	(((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_SWALLOW)
...
#define glyph_is_trap(glyph)							\
    (((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OTHERS &&			\
     ((glyph) & GLYPH_MAIN_MASK) >= (GLYPH_CMAP_OFF+trap_to_defsym(1)) &&	\
     ((glyph) & GLYPH_MAIN_MASK) <  (GLYPH_CMAP_OFF+trap_to_defsym(1)+TRAPNUM))
#define glyph_is_cmap(glyph)						\
    (((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OTHERS &&		\
     ((glyph) & GLYPH_MAIN_MASK) >= GLYPH_CMAP_OFF &&			\
     ((glyph) & GLYPH_MAIN_MASK) <  (GLYPH_CMAP_OFF+MAXPCHARS))
#define glyph_is_warning(glyph)						\
    (((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OTHERS &&		\
     ((glyph) & GLYPH_MAIN_MASK) == GLYPH_WARNING_OFF)
#define glyph_is_cloud(glyph)						\
    (((glyph) & GLYPH_TYPE_MASK) == GLYPH_TYPE_OTHERS &&		\
     ((glyph) & GLYPH_MAIN_MASK) >= GLYPH_CLOUD_OFF &&			\
     ((glyph) & GLYPH_MAIN_MASK) <  (GLYPH_CLOUD_OFF + MAXCCHARS))
#endif /* DISPLAY_H */
