...
#define	MLT_SWALLOWER	4	/* mlink is swallowing mon */
	Bitfield(maligntyp, 2);	/* alignment type for priests and minions */
	Bitfield(has_name, 1);	/* has an attached name */
	Bitfield(mtraitskept, 1); /* this mon is linked to an obj; do not purge it */
	/* 1 free bits */
...
#define STRAT_ROOMGUARD	0x00800000L
...
#define STRAT_GOALY(s)	((xchar)((s & STRAT_YMASK) >> 8))
	...
	struct xdat *mextra;	/* monster dependent info */
};
...
#define NAME(mtmp)	((char *)get_xdat_mon((mtmp), XDAT_NAME))
